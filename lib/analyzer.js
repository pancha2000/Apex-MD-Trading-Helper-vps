/**
 * ═══════════════════════════════════════════════════════════════
 *  APEX-MD  ·  lib/analyzer.js  ·  v7 PRO VVIP — Institutional Edition
 *  ─────────────────────────────────────────────────────────────
 *  UPGRADES OVER PREVIOUS VERSION:
 *
 *  1. 🧠 DYNAMIC WEIGHTS ENGINE (lib/dynamicWeights.js)
 *     All 60+ static score increments now multiplied by regime-aware
 *     weight multipliers (trend / oscillator / volume / priceAction).
 *     ADX < 20 → oscillators boosted, trend indicators cut in half.
 *     ADX > 25 → trend indicators boosted, oscillators reduced.
 *     ATR% overlays volatility regime on top.
 *
 *  2. 🐳 BYBIT CROSS-EXCHANGE VALIDATION (lib/multiExchange.js)
 *     Fetches live Bybit + Binance OB + volume data in parallel.
 *     Detects: spoof walls, OB divergence, single-exchange vol spikes.
 *     Returns a modifier (-3 to +3) applied AFTER core scoring.
 *
 *  3. 🤖 LOCAL AI MODEL LAYER (http://localhost:5000/predict)
 *     Sends last 120 candles to the Python FastAPI LSTM server.
 *     If confidence > 80%: ±2 modifier.  60–80%: ±1 modifier.
 *     Full try-catch — bot continues if Python server is offline.
 *
 *  All original fields preserved. Return shape unchanged.
 *  Regime metadata + exchange debug + AI result added to output.
 * ═══════════════════════════════════════════════════════════════
 */
...
function refineTradeParameters({
    ...
    if (mfi) {
        if (isLong && mfi.isBull && mfi.value < 25 && Math.abs(rEntry - cp) / cp < 0.003) {
            rEntry = cp * 1.001;
            refinements.push(`Entry: MFI Oversold (${mfi.value.toFixed(0)}) $${rEntry.toFixed(4)}`);
        }
        if (!isLong && mfi.isBear && mfi.value > 75 && Math.abs(rEntry - cp) / cp < 0.003) {
            rEntry = cp * 0.999;
            refinements.push(`Entry: MFI Overbought (${mfi.value.toFixed(0)}) $${rEntry.toFixed(4)}`);
        }
    }
    ...
    if (gannAngles) {
        ...
        if (gangOk && Math.abs(rEntry - cp) / cp < 0.004) {
            ...
            refinements.push(`Entry: Gann ${isLong ? 'Bull' : 'Bear'} $${rEntry.toFixed(4)}`);
        }
    }

    // A4. Advanced Candle Pattern
    if (advCandles && advCandles.pattern !== 'None') {
        const bullPats = ['Three White Soldiers','Morning Star','Tweezer Bottom','Bullish Harami','Piercing Line'];
        const bearPats = ['Three Black Crows','Evening Star','Tweezer Top','Bearish Harami','Dark Cloud Cover'];
        const isBullPat = bullPats.some(p => advCandles.pattern?.includes(p));
        const isBearPat = bearPats.some(p => advCandles.pattern?.includes(p));
        if ((isLong && isBullPat) || (!isLong && isBearPat)) {
            rEntry = isLong ? cp * 1.001 : cp * 0.999;
            refinements.push(`Entry: ${advCandles.pattern} $${rEntry.toFixed(4)}`);
        }
    }
    ...
    if (dynamicSR) {
        if (isLong  && dynamicSR.nearSupport  && dynamicSR.support) {
            ...
            if (Math.abs(sup - cp) / cp < 0.004) { rEntry = sup * 1.001; refinements.push(`Entry: Dynamic Support $${rEntry.toFixed(4)}`); }
        }
        if (!isLong && dynamicSR.nearResist && dynamicSR.resistance) {
            const res = parseFloat(dynamicSR.resistance);
            if (Math.abs(res - cp) / cp < 0.004) { rEntry = res * 0.999; refinements.push(`Entry: Dynamic Resist $${rEntry.toFixed(4)}`); }
        }
    }
    ...
    if (fvgData) {
        ...
        const nearest = fvgs.find(g => Math.abs(parseFloat(g.mid) - cp) / cp < 0.004 && !g.filled);
        if (nearest) { rEntry = parseFloat(nearest.mid); refinements.push(`Entry: FVG mid $${rEntry.toFixed(4)}`); }
    }

    // A8. MTF OB Confluence
    if (mtfOB?.confluenceZone) {
        const cz = mtfOB.confluenceZone;
        const czBot = parseFloat(cz.bottom), czTop = parseFloat(cz.top);
        if (isLong  && cz.type === 'BULLISH' && cp >= czBot * 0.998 && cp <= czTop * 1.002) { rEntry = czBot * 1.001; refinements.push(`Entry: MTF OB Confluence $${rEntry.toFixed(4)} 🔥`); }
        if (!isLong && cz.type === 'BEARISH' && cp >= czBot * 0.998 && cp <= czTop * 1.002) { rEntry = czTop * 0.999; refinements.push(`Entry: MTF OB Confluence $${rEntry.toFixed(4)} 🔥`); }
    }

    // A9. OTE Zone
    if (fibLevels?.isAtOTE) {
        const ote618 = parseFloat(fibLevels.key618), ote786 = parseFloat(fibLevels.key786);
        const d618 = Math.abs(cp - ote618) / cp, d786 = Math.abs(cp - ote786) / cp;
        if (isLong) {
            if (d618 < 0.005) { rEntry = ote618 * 1.001; refinements.push(`Entry: OTE 61.8% $${rEntry.toFixed(4)}`); }
            else if (d786 < 0.005) { rEntry = ote786 * 1.001; refinements.push(`Entry: OTE 78.6% $${rEntry.toFixed(4)}`); }
        } else {
            if (d618 < 0.005) { rEntry = ote618 * 0.999; refinements.push(`Entry: OTE 61.8% SHORT $${rEntry.toFixed(4)}`); }
            else if (d786 < 0.005) { rEntry = ote786 * 0.999; refinements.push(`Entry: OTE 78.6% SHORT $${rEntry.toFixed(4)}`); }
        }
    }

    // A10. Fib Confluence Zone
    if (fibConf?.hasConfluence && fibConf.zone) {
        const fibZone = parseFloat(fibConf.zone);
        if (Math.abs(fibZone - cp) / cp < 0.005) { rEntry = fibZone; refinements.push(`Entry: Fib Confluence $${rEntry.toFixed(4)} (${fibConf.count}×)`); }
    }

    // A11. Kijun-sen retest
    if (ichimoku?.kijun) {
        const kijun = parseFloat(ichimoku.kijun);
        if (Math.abs(kijun - cp) / cp < 0.003) { rEntry = isLong ? kijun * 1.001 : kijun * 0.999; refinements.push(`Entry: Kijun retest $${rEntry.toFixed(4)}`); }
    }

    // A12. Breaker Block (highest priority — last write wins)
    ...
    if (supertrend?.supertrendLevel && parseFloat(supertrend.supertrendLevel) > 0) {
        const stl = parseFloat(supertrend.supertrendLevel);
        if (isLong  && supertrend.isBull) slTighten(stl * 0.998, 'Supertrend');
        if (!isLong && supertrend.isBear) slTighten(stl * 1.002, 'Supertrend');
    }
    if (ichimoku?.kijun) { const k = parseFloat(ichimoku.kijun); slTighten(isLong ? k * 0.997 : k * 1.003, 'Kijun SL'); }
    ...
    if (isLong  && breakers?.bullishBreaker) slTighten(parseFloat(breakers.bullishBreaker.bottom) * 0.997, 'Breaker SL');
    if (!isLong && breakers?.bearishBreaker) slTighten(parseFloat(breakers.bearishBreaker.top)    * 1.003, 'Breaker SL');
    if (volNodes?.lvnZones?.length > 0) {
        const lvns = volNodes.lvnZones.map(z => parseFloat(z)).filter(isFinite);
        if (isLong)  { const lb = lvns.filter(z => z < rEntry).sort((a,b) => b-a)[0]; if (lb) slTighten(lb * 0.997, 'LVN SL'); }
        else         { const la = lvns.filter(z => z > rEntry).sort((a,b) => a-b)[0]; if (la) slTighten(la * 1.003, 'LVN SL'); }
    }
    if (equalHL) {
        if (isLong  && equalHL.eql) slTighten(parseFloat(equalHL.eql.level) * 0.995, 'EQL Guard SL');
        if (!isLong && equalHL.eqh) slTighten(parseFloat(equalHL.eqh.level) * 1.005, 'EQH Guard SL');
    }
    if (bbands) { const bbSL = isLong ? parseFloat(bbands.lower) : parseFloat(bbands.upper); if (bbSL && isFinite(bbSL)) slTighten(isLong ? bbSL * 0.998 : bbSL * 1.002, 'BB Band SL'); }
    if (ichimoku?.tenkan) {
        const t = parseFloat(ichimoku.tenkan);
        if (isLong  && t < rEntry * 0.995) slTighten(t * 0.997, 'Tenkan SL');
        if (!isLong && t > rEntry * 1.005) slTighten(t * 1.003, 'Tenkan SL');
    }
    if (adxData) {
        const adxVal = parseFloat(adxData.adx || adxData.value || 0);
        if (adxVal > 35 && rvol?.signal === 'EXTREME') slWiden(isLong ? rSL * 0.993 : rSL * 1.007, 'High Vol SL');
        else if (adxVal < 18) slTighten(isLong ? rSL * 1.004 : rSL * 0.996, 'Low Vol SL');
    }
    if (session?.quality === 'CAUTION')                  slWiden(isLong ? rSL * 0.996 : rSL * 1.004, 'Asian Session SL');
    if (wyckoff?.phase === 'SPRING'  && isLong)          slWiden(rSL * 0.996, 'Spring SL');
    if (wyckoff?.phase === 'UTAD'    && !isLong)         slWiden(rSL * 1.004, 'UTAD SL');
    if (mfi && ((isLong && mfi.isBear) || (!isLong && mfi.isBull))) slWiden(isLong ? rSL * 0.996 : rSL * 1.004, 'MFI Conflict Buffer SL');
    if (rvol?.signal === 'EXTREME')                      slWiden(isLong ? rSL * 0.994 : rSL * 1.006, 'RVOL Extreme Buffer SL');

    // ── SECTION C: TP REFINEMENT ──────────────────────────────
    const risk = Math.abs(rEntry - rSL) || atr * 1.5;
    const snapTP1 = (lv, lb, tol = 0.04) => {
        if (!lv || !isFinite(lv)) return;
        if (isLong  && lv > rEntry && Math.abs(lv - rTP1) / rTP1 < tol) { rTP1 = lv; rTP1Label = lb; refinements.push(`TP1 → ${lb} $${rTP1.toFixed(4)}`); }
        if (!isLong && lv < rEntry && Math.abs(lv - rTP1) / rTP1 < tol) { rTP1 = lv; rTP1Label = lb; refinements.push(`TP1 → ${lb} $${rTP1.toFixed(4)}`); }
    };
    const snapTP2 = (lv, lb, tol = 0.07) => {
        if (!lv || !isFinite(lv)) return;
        if (isLong  && lv > rTP1 && Math.abs(lv - rTP2) / rTP2 < tol) { rTP2 = lv; rTP2Label = lb; refinements.push(`TP2 → ${lb} $${rTP2.toFixed(4)}`); }
        if (!isLong && lv < rTP1 && Math.abs(lv - rTP2) / rTP2 < tol) { rTP2 = lv; rTP2Label = lb; refinements.push(`TP2 → ${lb} $${rTP2.toFixed(4)}`); }
    };
    const snapTP3 = (lv, lb) => {
        if (!lv || !isFinite(lv)) return;
        if (isLong  && lv > rTP2 && lv < rTP3) { rTP3 = lv; rTP3Label = lb; refinements.push(`TP3 → ${lb} $${rTP3.toFixed(4)}`); }
        if (!isLong && lv < rTP2 && lv > rTP3) { rTP3 = lv; rTP3Label = lb; refinements.push(`TP3 → ${lb} $${rTP3.toFixed(4)}`); }
    };

    if (pivots) {
        if (isLong) { snapTP1(parseFloat(pivots.R1)*0.999,'Pivot R1'); snapTP2(parseFloat(pivots.R2)*0.999,'Pivot R2'); snapTP3(parseFloat(pivots.R3),'Pivot R3 🎯'); }
        else        { snapTP1(parseFloat(pivots.S1)*1.001,'Pivot S1'); snapTP2(parseFloat(pivots.S2)*1.001,'Pivot S2'); snapTP3(parseFloat(pivots.S3),'Pivot S3 🎯'); }
    }
    if (ichimoku?.kijun)  { snapTP1(isLong ? parseFloat(ichimoku.kijun)*0.999 : parseFloat(ichimoku.kijun)*1.001, 'Kijun TP'); }
    if (ichimoku?.tenkan) {
        const t = parseFloat(ichimoku.tenkan);
        if (isLong  && t > rEntry*1.003) snapTP1(t*0.999, 'Tenkan TP1');
        if (!isLong && t < rEntry*0.997) snapTP1(t*1.001, 'Tenkan TP1');
    }
    if (ichimoku && !ichimoku.inCloud) {
    ...
    if (fvgData) {
        const bFVGs = (fvgData.bullFVGs||[]).filter(g=>!g.filled), rFVGs = (fvgData.bearFVGs||[]).filter(g=>!g.filled);
        if (isLong) { const a = bFVGs.filter(g=>parseFloat(g.mid)>rEntry).sort((a,b)=>parseFloat(a.mid)-parseFloat(b.mid)); if(a[0])snapTP1(parseFloat(a[0].mid),'FVG Fill TP1'); if(a[1])snapTP2(parseFloat(a[1].mid),'FVG Fill TP2'); }
        else        { const b = rFVGs.filter(g=>parseFloat(g.mid)<rEntry).sort((a,b)=>parseFloat(b.mid)-parseFloat(a.mid)); if(b[0])snapTP1(parseFloat(b[0].mid),'FVG Fill TP1'); if(b[1])snapTP2(parseFloat(b[1].mid),'FVG Fill TP2'); }
    }
    if (equalHL) {
        if (isLong  && equalHL.eqh) { const p = parseFloat(equalHL.eqh.level)*0.997; if(p>rEntry){if(p<rTP1*1.03){rTP1=p;rTP1Label='EQH Pool TP1';refinements.push(`TP1 → EQH $${rTP1.toFixed(4)}`);}else snapTP2(p,'EQH Pool TP2');} }
        if (!isLong && equalHL.eql) { const p = parseFloat(equalHL.eql.level)*1.003; if(p<rEntry){if(p>rTP1*0.97){rTP1=p;rTP1Label='EQL Pool TP1';refinements.push(`TP1 → EQL $${rTP1.toFixed(4)}`);}else snapTP2(p,'EQL Pool TP2');} }
    }
    if (dynamicSR) {
    ...
    if (fibLevels?.levels) {
        if (isLong) { const e1=parseFloat(fibLevels.ext1272),e2=parseFloat(fibLevels.ext1618),e3=parseFloat(fibLevels.ext2618); if(e1>rEntry)snapTP1(e1,'Fib 1.272 TP1'); if(e2>rTP1)snapTP2(e2,'Fib 1.618 TP2 🎯'); if(e3>rTP2)snapTP3(e3,'Fib 2.618 TP3 🚀'); }
        else        { const r1=parseFloat(fibLevels.key618),r2=parseFloat(fibLevels.key382); if(r1<rEntry)snapTP1(r1,'Fib 61.8% TP1'); if(r2<rTP1)snapTP2(r2,'Fib 38.2% TP2'); }
    }
    if (breakers) {
        if (isLong  && breakers.bearishBreaker) { const b=parseFloat(breakers.bearishBreaker.bottom); if(b>rEntry)snapTP2(b*0.997,'Breaker Barrier TP'); }
        if (!isLong && breakers.bullishBreaker) { const t=parseFloat(breakers.bullishBreaker.top);    if(t<rEntry)snapTP2(t*1.003,'Breaker Barrier TP'); }
    }
    if (mtfOB?.confluenceZone) {
        const cz=mtfOB.confluenceZone, czBot=parseFloat(cz.bottom), czTop=parseFloat(cz.top);
        if (isLong  && czBot>rEntry)  snapTP2(czBot,'MTF OB Confluence TP 🔥');
        if (!isLong && czTop<rEntry)  snapTP2(czTop,'MTF OB Confluence TP 🔥');
    }
    if (cmeGap?.hasGap && cmeGap.gapMid) {
        const gm=parseFloat(cmeGap.gapMid);
        if (isLong  && gm>rEntry) snapTP3(gm,'CME Gap Fill 🎯');
        if (!isLong && gm<rEntry) snapTP3(gm,'CME Gap Fill 🎯');
    }
    if (weeklyTgts) { const wt=isLong?parseFloat(weeklyTgts.weeklyHigh):parseFloat(weeklyTgts.weeklyLow); if(wt&&isFinite(wt))snapTP3(wt,'Weekly Target TP3 📅'); }
    if (gannAngles?.targetAbove && isLong)  snapTP3(parseFloat(gannAngles.targetAbove),'Gann Extension TP3');
    if (gannAngles?.targetBelow && !isLong) snapTP3(parseFloat(gannAngles.targetBelow),'Gann Extension TP3');
    if (dynamicSR) {
...
//  Returns: { prediction, confidence } or null on failure
// ══════════════════════════════════════════════════════════════
async function _callAIModel(coin, candles) {
    try {
        ...
        const { prediction, confidence, source } = response.data;
        if (!prediction || typeof confidence !== 'number') return null;

        return { prediction, confidence, source: source || 'lstm' };
    } catch (err) {
        // Silently absorb all errors — AI layer is bonus, not critical
        if (err.code === 'ECONNREFUSED') {
            // Python server offline — expected when not running
        } else {
...
// ── AI Score Modifier Builder ─────────────────────────────────
// confidence ≥ 80% AND matches direction: +2
// confidence ≥ 60% AND matches direction: +1
// confidence ≥ 80% BUT opposes direction: -2
// confidence ≥ 60% BUT opposes direction: -1
// confidence < 60% or null:               0
function _buildAIModifier(aiResult, direction) {
    if (!aiResult || aiResult.confidence < 60) {
        return { modifier: 0, aiLabel: 'AI_LOW_CONF ⚪' };
    }

    const aligns = (direction === 'LONG'  && aiResult.prediction === 'Bullish') ||
                   (direction === 'SHORT' && aiResult.prediction === 'Bearish');
    const highConf = aiResult.confidence >= 80;

    const modifier  = aligns ? (highConf ? 2 : 1) : (highConf ? -2 : -1);
    const direction_emoji = aligns ? '✅' : '❌';
    const confLabel = highConf ? 'HIGH' : 'MOD';
    const aiLabel   = `AI ${aiResult.prediction} ${aiResult.confidence.toFixed(0)}% [${confLabel}] ${direction_emoji}(${modifier >= 0 ? '+' : ''}${modifier})`;

    return { modifier, aiLabel };
}

// ══════════════════════════════════════════════════════════════
//  MASTER ANALYSIS FUNCTION (public entry point)
// ══════════════════════════════════════════════════════════════
async function run14FactorAnalysis(coin, timeframe = '15m') {
    // Master 30s timeout guard
...
async function _run14FactorAnalysisImpl(coin, timeframe = '15m') {
    ...
    // ── 🧠 DYNAMIC WEIGHTS (NEW) ─────────────────────────────
    // Calculate once per analysis run — feeds into every scoring line below.
    const dynResult = getDynamicWeights(currentCandles.slice(-100));
    const weights   = dynResult.weights;
    // weights.trend / .oscillator / .volume / .priceAction
    // w(pts, weight) multiplies points by the regime multiplier
    ...
    //     🧠 All points now multiplied by dynamic regime weights:
    //        w(pts, weights.trend)       — trend-following signals
    //        w(pts, weights.oscillator)  — oscillator/mean-revert signals
    //        w(pts, weights.volume)      — volume/volatility signals
    //        w(pts, weights.priceAction) — SMC / price structure signals
    // ══════════════════════════════════════════════════════════
    ...
    // ════════════════════════════════════════════
    // v6 BIG-PROFIT SCORING
    // ════════════════════════════════════════════

    // BB Squeeze Explosion
    ...
    // Momentum Shift (unified: ROC + MFI + StochRSI + CCI)
    if (momentumShift.isBull) {
        const pts = momentumShift.bullSignals >= 6 ? 3 : momentumShift.bullSignals >= 5 ? 2 : 1;
        longScore  += w(pts, weights.oscillator); longR.push(`Momentum BULL ${momentumShift.bullSignals}/6 ⚡🟢`);
    }
    if (momentumShift.isBear) {
        const pts = momentumShift.bearSignals >= 6 ? 3 : momentumShift.bearSignals >= 5 ? 2 : 1;
        shortScore += w(pts, weights.oscillator); shortR.push(`Momentum BEAR ${momentumShift.bearSignals}/6 ⚡🔴`);
    }

    // ════════════════════════════════════════════
    // ⚡ 5m SNIPER LAYER SCORING
    // ════════════════════════════════════════════
    ...
    // ══════════════════════════════════════════════════════════
    //  🐳 BYBIT CROSS-EXCHANGE CONFLUENCE (NEW)
    //  Fetches live Bybit OB + volume in parallel.
    //  Modifier range: -3 to +3, applied to WINNING side.
    // ══════════════════════════════════════════════════════════
    const smartDir0    = longScore >= shortScore ? 'LONG' : 'SHORT';
    const bybitResult  = await getBybitConfluence(coin, smartDir0);
    ...
    // ══════════════════════════════════════════════════════════
    //  🤖 LOCAL AI MODEL CALL (NEW)
    //  POST http://localhost:5000/predict
    //  Wrapped in try-catch — bot never crashes if offline.
    //  confidence > 80% → ±2 pts   |   60-80% → ±1 pt
    // ══════════════════════════════════════════════════════════
    let aiResult = null;
    let aiLabel  = 'AI_OFFLINE ⚪';
    let aiModifier = 0;

    try {
        aiResult = await _callAIModel(coin, currentCandles);
        if (aiResult) {
            const smartDir1 = longScore >= shortScore ? 'LONG' : 'SHORT';
            const built     = _buildAIModifier(aiResult, smartDir1);
            aiModifier      = built.modifier;
            aiLabel         = built.aiLabel;

            if (aiModifier !== 0) {
                if (smartDir1 === 'LONG') {
                    longScore  += aiModifier;
                    longR.push(aiLabel);
                } else {
                    shortScore += aiModifier;
                    shortR.push(aiLabel);
                }
            }
        }
    } catch (err) {
        // Belt-and-suspenders: _callAIModel already swallows errors, but just in case
        console.warn(`[analyzer] AI modifier failed: ${err.message}`);
    }
    ...
    const confChecks = {
        ...
        // NEW: AI + Bybit as confirmation factors
        aiConf:       aiResult !== null && aiModifier > 0,
    ...
    return {
        ...
        weights,             // { trend, oscillator, volume, priceAction }
        dynRegime: {
            trendRegime:  dynResult.trendRegime,
            volRegime:    dynResult.volRegime,
            adx:          dynResult.adx,
            atrPct:       dynResult.atrPct,
            regimeLabel:  dynResult.regimeLabel,
        },

        // ── 🐳 BYBIT CROSS-EXCHANGE (NEW) ─────────────────────
        bybitResult: {
            modifier:   bybitResult.modifier,
            regime:     bybitResult.regime,
            reasons:    bybitResult.reasons,
            available:  bybitResult.available,
        },
        ...
        aiResult: aiResult ? {
            ...
            source: 'offline', modifier: 0, label: aiLabel,
        },
    };
}
