...
function refineTradeParameters({
    ...
    // ── ENTRY PROXIMITY HARD CAP (Fix: prevents 3-day limit orders) ──────────────
    // Entry must stay within max(2.0%, 2×ATR%) of current price.
    // Entries further away fill after days/never — LONG pullback entries OK to be
    // below current price, SHORT throwback entries OK to be above, but within limit.
    if (atr > 0 && cp > 0) {
        const atrPct = (atr / cp) * 100;
        const maxDrift = Math.max(2.0, atrPct * 2.0); // e.g. ATR=0.5% → max 2% drift
        const driftPct = Math.abs(rEntry - cp) / cp * 100;
        if (driftPct > maxDrift) {
            const capped = isLong
                ? cp * (1 - Math.min(maxDrift, 2.0) / 100)
                : cp * (1 + Math.min(maxDrift, 2.0) / 100);
            refinements.push(`⚠️ Entry capped: was ${driftPct.toFixed(1)}% away → now ${(Math.abs(capped-cp)/cp*100).toFixed(1)}% ($${capped.toFixed(4)})`);
            rEntry = capped;
        }
    }
    ...
    // Section D: Safety validation + SL/TP quality guards
    const safeRisk = (atr > 0 ? atr * 1.5 : rEntry * 0.015); // fallback risk = 1.5% of entry
    const riskNow  = Math.abs(rEntry - rSL);

    // SL minimum distance: never closer than 0.8× ATR (anti-noise stop)
    const minSLDist = atr > 0 ? atr * 0.8 : rEntry * 0.008;
    if (isLong) {
        if (!isFinite(rSL)||isNaN(rSL)||rSL>=rEntry) rSL = rEntry - safeRisk;
        if ((rEntry - rSL) < minSLDist) { rSL = rEntry - minSLDist; refinements.push(`SL widened to min-dist ($${rSL.toFixed(4)})`); }
    } else {
        if (!isFinite(rSL)||isNaN(rSL)||rSL<=rEntry) rSL = rEntry + safeRisk;
        if ((rSL - rEntry) < minSLDist) { rSL = rEntry + minSLDist; refinements.push(`SL widened to min-dist ($${rSL.toFixed(4)})`); }
    }

    const finalRisk = Math.abs(rEntry - rSL);

    // TP minimum RRR: TP1 ≥ 1.5:1, TP2 ≥ 2.5:1, TP3 ≥ 4:1
    if (isLong) {
        if (!isFinite(rTP1)||isNaN(rTP1)||rTP1<=rEntry) rTP1 = rEntry + finalRisk * 2.0;
        if ((rTP1 - rEntry) < finalRisk * 1.5) { rTP1 = rEntry + finalRisk * 2.0; refinements.push(`TP1 raised to min 2:1 RRR ($${rTP1.toFixed(4)})`); }
        if (!isFinite(rTP2)||isNaN(rTP2)||rTP2<=rTP1) rTP2 = rTP1 + finalRisk;
        if ((rTP2 - rEntry) < finalRisk * 2.5) { rTP2 = rEntry + finalRisk * 3.0; }
        if (!isFinite(rTP3)||isNaN(rTP3)||rTP3<=rTP2) rTP3 = rTP2 + finalRisk * 2;
        if ((rTP3 - rEntry) < finalRisk * 4.0) { rTP3 = rEntry + finalRisk * 5.0; }
    } else {
        if (!isFinite(rTP1)||isNaN(rTP1)||rTP1>=rEntry) rTP1 = rEntry - finalRisk * 2.0;
        if ((rEntry - rTP1) < finalRisk * 1.5) { rTP1 = rEntry - finalRisk * 2.0; refinements.push(`TP1 lowered to min 2:1 RRR ($${rTP1.toFixed(4)})`); }
        if (!isFinite(rTP2)||isNaN(rTP2)||rTP2>=rTP1) rTP2 = rTP1 - finalRisk;
        if ((rEntry - rTP2) < finalRisk * 2.5) { rTP2 = rEntry - finalRisk * 3.0; }
        if (!isFinite(rTP3)||isNaN(rTP3)||rTP3>=rTP2) rTP3 = rTP2 - finalRisk * 2;
        if ((rEntry - rTP3) < finalRisk * 4.0) { rTP3 = rEntry - finalRisk * 5.0; }
    }
...
async function _run14FactorAnalysisImpl(coin, timeframe = '15m') {
    ...
    if (mtfFibEntry?.atZone && mtfFibEntry.nearestZone?.strength === 'STRONG') {
        ...
        // REAL bypass (70%+ to TP1) = penalise heavily, RRR is too poor
        if (direction === 'LONG')  { longScore  = Math.max(0, longScore  - 8); longR.push('⚠️ Near TP1 — RRR poor, wait for pullback'); }
        else                       { shortScore = Math.max(0, shortScore - 8); shortR.push('⚠️ Near TP1 — RRR poor, wait for pullback'); }
    } else if (mtfFibEntry?.nearestZone && !mtfFibEntry.atZone) {
        // Price between entry zone and TP (momentum continuation) — small bonus
        const zoneDir = direction === 'LONG'
            ? (currentPrice > mtfFibEntry.nearestZone.price)
            : (currentPrice < mtfFibEntry.nearestZone.price);
        if (zoneDir) {
            if (direction === 'LONG')  { longScore  += w(1, weights.priceAction); longR.push('▶️ Fib Zone Cleared — Continuation'); }
            else                       { shortScore += w(1, weights.priceAction); shortR.push('▶️ Fib Zone Cleared — Continuation'); }
        }
    }
    ...
    let sessionBlocked = isAsianSession && (signalGrade === 'C' || signalGrade === 'D');
    let bestReasons   = (bestDirection === 'LONG' ? longR : shortR).join(', ') || 'None';

    // ── REAL ZONE BYPASS (70%+ to TP1) — HARD BLOCK ──────────────────────────
    // Only block when price has nearly reached TP1 already (RRR < 0.3:1 remaining).
    // This is different from "price passed entry zone going toward TP" (normal trade).
    let zoneBypassed = mtfFibEntry?.zoneBypassed === true;
    if (zoneBypassed) {
        // Trade is 70%+ complete — RRR too poor. Block high grades.
        if (signalGrade === 'A+') { signalGrade = 'C'; signalGradeEmoji = '⏳'; signalGradeLabel = 'WAIT — Near TP1'; }
        else if (signalGrade === 'A') { signalGrade = 'D'; signalGradeEmoji = '⚠️'; signalGradeLabel = 'AVOID — Near TP1'; }
    }

    // ── ENTRY DISTANCE GRADE PENALTY ─────────────────────────────────────────
    // If deepEntry says SKIP (no viable nearby zone), reduce grade.
    const _entryDist = Math.abs(parseFloat(refined.entry) - currentPrice) / currentPrice * 100;
    const _atrPctNow = atrVal > 0 ? (atrVal / currentPrice) * 100 : 0.5;
    if (deepEntry?.decision === 'SKIP') {
        if (signalGrade === 'A+') { signalGrade = 'B'; signalGradeEmoji = '🥈'; signalGradeLabel = 'STANDARD'; }
        else if (signalGrade === 'A') { signalGrade = 'C'; signalGradeEmoji = '🥉'; signalGradeLabel = 'LOW QUALITY'; }
        else if (signalGrade === 'B') { signalGrade = 'D'; signalGradeEmoji = '⚠️'; signalGradeLabel = 'AVOID — No Nearby Zone'; }
    } else if (deepEntry?.decision === 'LIMIT_FAR' && _entryDist > _atrPctNow * 3) {
        if (signalGrade === 'A+') { signalGrade = 'A'; signalGradeEmoji = '🥇'; signalGradeLabel = 'HIGH QUALITY'; }
        else if (signalGrade === 'A') { signalGrade = 'B'; signalGradeEmoji = '🥈'; signalGradeLabel = 'STANDARD'; }
    }

    // ── CONTINUATION ENTRY: use deepEntry optimal zone ────────────────────────
    // When price has passed entry zone but trade is still in progress (NOT bypass),
    // deepEntry finds the closest HIGH-CONFLUENCE zone — use that as the real entry.
    // This prevents refined.entry from being placed at a far Fib zone.
    if (deepEntry?.decision === 'MARKET' || deepEntry?.decision === 'LIMIT_NEAR') {
        const deepPrice = deepEntry.optimalEntry;
        if (deepPrice && isFinite(deepPrice) && deepPrice > 0) {
            const deepDist = Math.abs(deepPrice - currentPrice) / currentPrice * 100;
            const refDist  = Math.abs(parseFloat(refined.entry) - currentPrice) / currentPrice * 100;
            if (deepDist < refDist - 0.5) {
                refined.entry = deepPrice.toFixed(4);
                const newEntry = deepPrice;
                const curSL    = parseFloat(refined.sl);
                const slValid  = bestDirection === 'LONG' ? curSL < newEntry : curSL > newEntry;
                if (!slValid) {
                    refined.sl      = bestDirection === 'LONG'
                        ? (newEntry - atrVal * 1.5).toFixed(4)
                        : (newEntry + atrVal * 1.5).toFixed(4);
                    refined.slLabel = 'ATR 1.5x (auto)';
                }
            }
        }
    }
