...
async function setMode(mode){const msg=document.getElementById('mode-msg');msg.textContent='⏳ Saving...';msg.style.color='var(--text2)';try{const r=await fetch('/app/api/trading-mode',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({mode})});const d=await r.json();if(d.ok){msg.textContent=mode==='auto_trade'?'✅ Auto Trade enabled':'✅ Signals Only enabled';msg.style.color='var(--green)';setTimeout(()=>location.reload(),900);}else{msg.textContent='❌ '+d.error;msg.style.color='var(--red)';}}catch(e){msg.textContent='❌ Network error';msg.style.color='var(--red)';}}
    ...
    app.post('/app/api/scanbacktest', saasAuth.requireUserAuth, async (req, res) => {
        try {
            ...
            console.log(`[SCANBT] ${req.saasUser.username} → ${timeframe} top-${limit}`);
            const binanceLib = require('./lib/binance');
            const indLib     = require('./lib/indicators');
            const smcLib     = require('./lib/smartmoney');
            const allCoins   = binanceLib.isReady()
                ? binanceLib.getWatchedCoins().slice(0,limit)
                : ['BTCUSDT','ETHUSDT','SOLUSDT','BNBUSDT','XRPUSDT','ADAUSDT','AVAXUSDT','DOTUSDT','LINKUSDT','MATICUSDT','ATOMUSDT','NEARUSDT','LTCUSDT','DOGEUSDT','UNIUSDT'].slice(0,limit);
            const coins = allCoins.filter(c => !_STABLES.has(c));
            const results = [];
            for (const coin of coins) {
                try {
                    ...
                    while(i<candles.length-25){
                        const slice=candles.slice(Math.max(0,i-60),i);
                        if(slice.length<50){i++;continue;}
                        const cp=parseFloat(slice[slice.length-1][4]);
                        const atr=parseFloat(indLib.calculateATR(slice.slice(-30),14));
                        const adxR=indLib.calculateADX(slice.slice(-30));
                        ...
                        if((mSMC.sweep||'').includes('Bullish'))ls+=2;if((mSMC.sweep||'').includes('Bearish'))ss+=2;
                        if((mSMC.choch||'').includes('Bullish'))ls+=2;if((mSMC.choch||'').includes('Bearish'))ss+=2;
                        if(Math.max(ls,ss)<6){i++;continue;}
                        const isLong=ls>=ss;
                        const entry=cp,sl2=isLong?cp-atr*2:cp+atr*2;
                        const tp2=isLong?cp+atr*3:cp-atr*3;
                        let hit=false;
                        for(let j=i;j<Math.min(i+100,candles.length);j++){
                            const hi=parseFloat(candles[j][2]),lo=parseFloat(candles[j][3]);
                            if(isLong){if(lo<=sl2){losses++;equity-=1;hit=true;break;}if(hi>=tp2){wins++;equity+=3;hit=true;break;}}
                            else{if(hi>=sl2){losses++;equity-=1;hit=true;break;}if(lo<=tp2){wins++;equity+=3;hit=true;break;}}
                        }
                        i+=15;
                    }
                    const total=wins+losses;
                    ...
                    results.push({ coin:coin.replace('USDT',''), total, wins, losses, winRate, pf, netR:(equity).toFixed(1),
                        grade:pfNum>=2?'Excellent':pfNum>=1.5?'Good':pfNum>=1?'Marginal':'Poor',
                        gradeColor:pfNum>=2?'#00e676':pfNum>=1.5?'#00c8ff':pfNum>=1?'#ffab00':'#ff3355',
            ...
            res.json({ok:true,results,timeframe,scanned:coins.length});
        } catch(e){ res.status(500).json({ok:false,error:e.message}); }
    });

    // ════════════════════════════════════════════════════════════════════
    //  MARKET INTEL API  GET /app/api/market-intel
    // ════════════════════════════════════════════════════════════════════
    app.get('/app/api/market-intel', saasAuth.requireUserAuth, async (req, res) => {
        try {
            ...
            const sent = await _withTimeout(binance.getMarketSentiment(), 12000, fallback);
            let topFunding = [];
            try {
                ...
                const fr = await axios2.get('https://fapi.binance.com/fapi/v1/premiumIndex',{timeout:8000});
                topFunding = WATCH.map(sym=>{
                    const d=fr.data.find(x=>x.symbol===sym);
                    if(!d) return null;
                    const rate=(parseFloat(d.lastFundingRate)*100).toFixed(4);
                    const rn=parseFloat(rate);
                    if(Math.abs(rn)<0.05) return null;
                    return {coin:sym.replace('USDT',''),rate,signal:rn>0.1?'Longs heavy → SHORT squeeze':rn<-0.1?'Shorts heavy → LONG squeeze':rn>0?'Longs elevated':'Shorts elevated'};
                }).filter(Boolean).sort((a,b)=>Math.abs(parseFloat(b.rate))-Math.abs(parseFloat(a.rate)));
            } catch(_){}
            res.json({ok:true,sent,topFunding});
        } catch(e){ res.status(500).json({ok:false,error:e.message}); }
    });
    ...
    app.post('/app/api/paper/open', saasAuth.requireUserAuth, async (req, res) => {
        try {
            ...
            await db.saveTrade({ userJid:waJid, userId:req.saasUser.userId, coin:coinFull, type:'future', direction, entry:parseFloat(entry), tp:parseFloat(tp2), tp1:null, tp2:parseFloat(tp2), sl:parseFloat(sl), status:'active', orderType:'MARKET', isPaper:true, source:'WEB', leverage:parseInt(leverage)||10, score:0, timeframe:'15m' });
            res.json({ok:true});
        } catch(e){ res.status(500).json({ok:false,error:e.message}); }
    });

    app.post('/app/api/paper/close/:id', saasAuth.requireUserAuth, async (req, res) => {
        try {
            const fullUser = await db.getSaasUserById(req.saasUser.userId);
            const waJid = fullUser?.whatsappJid;
            const trade = await db.Trade.findById(req.params.id);
            ...
            const priceRes = await require('axios').get(`https://api.binance.com/api/v3/ticker/price?symbol=${trade.coin}`,{timeout:5000});
            const curP = parseFloat(priceRes.data.price);
            const isLong = trade.direction==='LONG';
            const priceDiff = isLong?curP-trade.entry:trade.entry-curP;
            const profit = priceDiff*(trade.quantity||0.01);
    ...
    app.post('/app/api/alerts', saasAuth.requireUserAuth, async (req, res) => {
        try {
            ...
            const Alert2 = mongoose2.models.Alert || require('./plugins/alerts').Alert;
            const fullUser = await db.getSaasUserById(req.saasUser.userId);
            const waJid = fullUser?.whatsappJid;
            if (!waJid) return res.status(400).json({ok:false,error:'WhatsApp not linked'});
            const { coin='', condition='above', targetPrice } = req.body;
            if (!coin||!targetPrice) return res.status(400).json({ok:false,error:'coin and targetPrice required'});
            const coinFull = (coin.toUpperCase().replace(/[^A-Z0-9]/g,'')+(coin.includes('USDT')?'':'USDT'));
            const count = await Alert2.countDocuments({userJid:waJid,triggered:false});
            if (count>=20) return res.status(400).json({ok:false,error:'Max 20 alerts'});
            await Alert2.create({userJid:waJid,coin:coinFull,condition,targetPrice:parseFloat(targetPrice)});
            res.json({ok:true});
        } catch(e){ res.status(500).json({ok:false,error:e.message}); }
    });
    ...
    app.delete('/app/api/alerts/:id', saasAuth.requireUserAuth, async (req, res) => {
        try {
            const mongoose2 = require('mongoose');
            const Alert2 = mongoose2.models.Alert;
            if (!Alert2) return res.status(400).json({ok:false,error:'Alert model not loaded'});
            const fullUser = await db.getSaasUserById(req.saasUser.userId);
            const alert = await Alert2.findById(req.params.id);
            if (!alert||alert.userJid!==fullUser?.whatsappJid) return res.status(403).json({ok:false,error:'Not yours'});
            await Alert2.findByIdAndDelete(req.params.id);
            res.json({ok:true});
        } catch(e){ res.status(500).json({ok:false,error:e.message}); }
    });
    ...
    app.post('/app/api/watchlist', saasAuth.requireUserAuth, async (req, res) => {
        try {
            const mongoose2 = require('mongoose');
            const Watchlist2 = mongoose2.models.Watchlist;
            if (!Watchlist2) return res.status(400).json({ok:false,error:'Watchlist model not loaded yet. Send .watch command in WhatsApp first.'});
            const fullUser = await db.getSaasUserById(req.saasUser.userId);
            const waJid = fullUser?.whatsappJid;
            if (!waJid) return res.status(400).json({ok:false,error:'WhatsApp not linked'});
            const { coins=[] } = req.body;
            const toAdd = coins.map(c=>(c.toUpperCase().replace(/[^A-Z0-9]/g,'')+(c.includes('USDT')?'':'USDT'))).filter(c=>!_STABLES.has(c));
            if (!toAdd.length) return res.status(400).json({ok:false,error:'No valid coins'});
            let wl = await Watchlist2.findOne({userJid:waJid});
            if (!wl) wl = new Watchlist2({userJid:waJid,coins:[]});
    ...
    app.delete('/app/api/watchlist/:coin', saasAuth.requireUserAuth, async (req, res) => {
        try {
            const mongoose2 = require('mongoose');
            const Watchlist2 = mongoose2.models.Watchlist;
            if (!Watchlist2) return res.json({ok:true});
            const fullUser = await db.getSaasUserById(req.saasUser.userId);
            const waJid = fullUser?.whatsappJid;
            if (!waJid) return res.status(400).json({ok:false,error:'WhatsApp not linked'});
            const coin = req.params.coin.toUpperCase()+(req.params.coin.includes('USDT')?'':'USDT');
            await Watchlist2.findOneAndUpdate({userJid:waJid},{$pull:{coins:coin}});
            res.json({ok:true});
        } catch(e){ res.status(500).json({ok:false,error:e.message}); }
    });
    ...
    app.get('/app/api/watchlist/prices', saasAuth.requireUserAuth, async (req, res) => {
        try {
            ...
            const fullUser = await db.getSaasUserById(req.saasUser.userId);
            const waJid = fullUser?.whatsappJid;
            if (!waJid||!Watchlist2) return res.json({ok:true,prices:[]});
            const wl = await Watchlist2.findOne({userJid:waJid}).lean();
            if (!wl?.coins?.length) return res.json({ok:true,prices:[]});
            const axios2 = require('axios');
            const prices = await Promise.all(wl.coins.map(async coin=>{
                try {
                    const [p,c] = await Promise.all([
                        axios2.get('https://api.binance.com/api/v3/ticker/price?symbol='+coin,{timeout:5000}),
                        axios2.get('https://api.binance.com/api/v3/ticker/24hr?symbol='+coin,{timeout:5000}),
                    ]);
                    return {coin,price:p.data.price,change:c.data.priceChangePercent};
    ...
    // ════════════════════════════════════════════════════════════════════
    //  PAPER TRADING PAGE  GET /app/paper
    // ════════════════════════════════════════════════════════════════════
    app.get('/app/paper', saasAuth.requireUserAuth, async (req, res) => {
        const user = req.saasUser;
        let pb=0,ps=100,wins=0,losses=0,active=[],closed=[],waJid='';
        try {
            const fu = await db.getSaasUserById(user.userId);
            waJid = fu?.whatsappJid||'';
            if (waJid) {
                const wu = await db.getUser(waJid).catch(()=>null);
                if (wu){pb=wu.paperBalance||0;ps=wu.paperStartBalance||100;wins=wu.paperWins||0;losses=wu.paperLosses||0;}
                active = await db.Trade.find({userJid:waJid,isPaper:true,status:{$in:['active','pending']}}).sort({openTime:-1}).lean();
                closed = await db.Trade.find({userJid:waJid,isPaper:true,status:'closed'}).sort({closedAt:-1}).limit(30).lean();
            }
        } catch(_){}
        const total=wins+losses;
        const pnlAmt=pb-ps, pnlPct=ps>0?((pnlAmt/ps)*100).toFixed(1):'0.0';
        const noWa = !waJid ? '<div style="background:rgba(255,171,0,.08);border:1px solid rgba(255,171,0,.2);border-radius:10px;padding:13px 16px;margin-bottom:20px;font-size:.84rem;color:#ffab00">⚠️ WhatsApp not linked. <a href="/app/settings" style="color:#00c8ff">Link in Settings →</a></div>' : '';
        const aRows = active.length===0
            ? '<tr><td colspan="8" style="text-align:center;color:#5c87a8;padding:28px">No open paper trades</td></tr>'
            : active.map(t=>{
                const d=t.direction==='LONG'?'<span class="pill pill-long">▲ L</span>':'<span class="pill pill-short">▼ S</span>';
                return '<tr><td><b style="font-family:var(--font-mono)">'+t.coin.replace('USDT','')+'</b></td><td>'+d+'</td><td style="font-family:var(--font-mono)">'+fmtPrice(t.entry)+'</td><td style="font-family:var(--font-mono);color:#00e676">'+fmtPrice(t.tp2||t.tp)+'</td><td style="font-family:var(--font-mono);color:#ff3355">'+fmtPrice(t.sl)+'</td><td style="font-family:var(--font-mono)">'+(t.leverage||10)+'x</td><td>'+((Date.now()-new Date(t.openTime))/3600000).toFixed(1)+'h</td><td><button class="btn btn-danger btn-sm" onclick="closeP(\''+t._id+'\',\''+t.coin.replace('USDT','')+'\')">Close</button></td></tr>';
            }).join('');
        const cRows = closed.length===0
            ? '<tr><td colspan="6" style="text-align:center;color:#5c87a8;padding:20px">No history yet</td></tr>'
            : closed.map(t=>{
                const r=t.result==='WIN'?'<span class="pill pill-win">WIN</span>':t.result==='LOSS'?'<span class="pill pill-loss">LOSS</span>':'<span class="pill pill-be">B/E</span>';
                const p=t.paperProfit?(t.paperProfit>=0?'+':'')+t.paperProfit.toFixed(2):'—';
                return '<tr><td><b style="font-family:var(--font-mono)">'+t.coin.replace('USDT','')+'</b></td><td>'+(t.direction==='LONG'?'<span class="pill pill-long">▲ L</span>':'<span class="pill pill-short">▼ S</span>')+'</td><td style="font-family:var(--font-mono)">'+fmtPrice(t.entry)+'</td><td>'+r+'</td><td style="font-family:var(--font-mono);color:'+(t.paperProfit>0?'var(--green)':t.paperProfit<0?'var(--red)':'var(--text2)')+'">'+p+'</td><td style="font-size:.78rem;color:var(--text2)">'+(t.closedAt?new Date(t.closedAt).toLocaleDateString():'—')+'</td></tr>';
            }).join('');
...
<div class="wrap">
  ...
  ${noWa}
  <div class="g-stats">
    <div class="stat-card"><div class="stat-label">Paper Balance</div><div class="stat-val c-yellow">$${parseFloat(pb).toFixed(2)}</div><div class="stat-sub">Virtual wallet</div></div>
    <div class="stat-card"><div class="stat-label">Net P&L</div><div class="stat-val" style="color:${pnlAmt>=0?'var(--green)':'var(--red)'}">${pnlAmt>=0?'+':''}$${Math.abs(pnlAmt).toFixed(2)}</div><div class="stat-sub">${pnlAmt>=0?'+':''}${pnlPct}%</div></div>
    <div class="stat-card"><div class="stat-label">Win Rate</div><div class="stat-val c-green">${total>0?((wins/total)*100).toFixed(1)+'%':'—'}</div><div class="stat-sub">${wins}W / ${losses}L</div></div>
    <div class="stat-card"><div class="stat-label">Open Positions</div><div class="stat-val c-cyan">${active.length}</div><div class="stat-sub">Max 5</div></div>
  </div>
  <div class="panel" style="margin-bottom:18px;border-color:rgba(255,171,0,.2)">
    <div class="panel-head"><div class="panel-title" style="color:var(--yellow)">📄 Open Paper Trade</div></div>
    <div class="panel-body">
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:10px;margin-bottom:12px">
        ...
        <div><label class="field-label">Leverage</label><input type="number" id="pt-lev" class="inp" value="10" min="1" max="125"></div>
      </div>
      <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap">
        <button class="btn btn-warn" id="pt-btn">📄 Open Trade</button>
        <button class="btn btn-danger btn-sm" onclick="resetPaper()">🔄 Reset ($100)</button>
        <span id="pt-msg" style="font-size:.83rem"></span>
      </div>
    </div>
  </div>
  <div class="panel" style="margin-bottom:18px">
    <div class="panel-head"><div class="panel-title">⚡ Open Positions (${active.length})</div></div>
    <div class="panel-body" style="padding:0"><div class="tbl-wrap">
      <table><thead><tr><th>Coin</th><th>Dir</th><th>Entry</th><th>TP2</th><th>SL</th><th>Lev</th><th>Open</th><th>Action</th></tr></thead>
      <tbody id="paper-body">${aRows}</tbody></table>
    </div></div>
  </div>
  <div class="panel">
    <div class="panel-head"><div class="panel-title">📜 History (Last 30)</div></div>
    <div class="panel-body" style="padding:0"><div class="tbl-wrap">
      <table><thead><tr><th>Coin</th><th>Dir</th><th>Entry</th><th>Result</th><th>Profit</th><th>Date</th></tr></thead>
...
<div class="toast" id="toast"></div>
<script>
function showToast(m,t){var el=document.getElementById('toast');el.textContent=m;el.className='toast'+(t==='err'?' err':t==='info'?' info':'');el.style.display='block';clearTimeout(el._t);el._t=setTimeout(function(){el.style.display='none';},3000);}
document.getElementById('pt-coin').addEventListener('input',function(){this.value=this.value.toUpperCase().replace(/[^A-Z0-9]/g,'');});
document.getElementById('pt-btn').addEventListener('click',function(){
  var coin=document.getElementById('pt-coin').value.trim();
  var dir=document.getElementById('pt-dir').value;
  var entry=parseFloat(document.getElementById('pt-entry').value);
  var tp2=parseFloat(document.getElementById('pt-tp2').value);
  var sl=parseFloat(document.getElementById('pt-sl').value);
  var lev=parseInt(document.getElementById('pt-lev').value)||10;
  ...
  fetch('/app/api/paper/open',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({coin:coin,direction:dir,entry:entry,tp2:tp2,sl:sl,leverage:lev})})
  .then(function(r){return r.json();}).then(function(d){
    if(d.ok){msg.style.color='var(--green)';msg.textContent='✅ Opened!';setTimeout(function(){location.reload();},1200);}
    else{msg.style.color='var(--red)';msg.textContent='❌ '+(d.error||'Failed');}
  }).catch(function(){msg.style.color='var(--red)';msg.textContent='❌ Network error';});
});
function closeP(id,coin){
  if(!confirm('Close paper trade for '+coin+'?'))return;
  fetch('/app/api/paper/close/'+id,{method:'POST'}).then(function(r){return r.json();}).then(function(d){
    if(d.ok){showToast('✅ Closed');setTimeout(function(){location.reload();},800);}
    else showToast('❌ '+(d.error||'Failed'),'err');
  });
}
function resetPaper(){
  if(!confirm('Reset paper balance to $100?'))return;
  fetch('/app/api/paper/reset',{method:'POST'}).then(function(r){return r.json();}).then(function(d){
    if(d.ok){showToast('✅ Reset done');setTimeout(function(){location.reload();},800);}
    else showToast('❌ '+(d.error||'Failed'),'err');
  });
}
</script>`));
    ...
    // ════════════════════════════════════════════════════════════════════
    //  PRICE ALERTS PAGE  GET /app/alerts
    // ════════════════════════════════════════════════════════════════════
    app.get('/app/alerts', saasAuth.requireUserAuth, async (req, res) => {
        const user = req.saasUser;
        let active=[],triggered=[],waJid='';
        try {
            const fu = await db.getSaasUserById(user.userId);
            waJid = fu?.whatsappJid||'';
            const mongoose2 = require('mongoose');
            const Alert2 = mongoose2.models.Alert;
            if (waJid && Alert2) {
                active    = await Alert2.find({userJid:waJid,triggered:false}).sort({createdAt:-1}).lean();
                triggered = await Alert2.find({userJid:waJid,triggered:true}).sort({createdAt:-1}).limit(15).lean();
            }
        } catch(_){}
        const noWa = !waJid ? '<div style="background:rgba(255,171,0,.08);border:1px solid rgba(255,171,0,.2);border-radius:10px;padding:13px 16px;margin-bottom:20px;font-size:.84rem;color:#ffab00">⚠️ WhatsApp not linked — alerts fire to your WhatsApp. <a href="/app/settings" style="color:#00c8ff">Link →</a></div>' : '';
        const aRows = active.length===0
            ? '<tr><td colspan="5" style="text-align:center;color:#5c87a8;padding:28px">No active alerts</td></tr>'
            : active.map(a=>'<tr><td><b style="font-family:var(--font-mono)">'+a.coin.replace('USDT','')+'</b></td><td style="color:'+(a.condition==='above'?'var(--green)':'var(--red)')+'">'+( a.condition==='above'?'📈 Above':'📉 Below')+'</td><td style="font-family:var(--font-mono);font-weight:700">$'+parseFloat(a.targetPrice).toFixed(4)+'</td><td style="font-size:.78rem;color:var(--text2)">'+new Date(a.createdAt).toLocaleDateString()+'</td><td><button class="btn btn-danger btn-sm" onclick="delAlert(\''+a._id+'\')">Delete</button></td></tr>').join('');
        const tRows = triggered.length===0
            ? '<tr><td colspan="3" style="text-align:center;color:#5c87a8;padding:20px">No triggered alerts yet</td></tr>'
            : triggered.map(a=>'<tr><td style="font-family:var(--font-mono)">'+a.coin.replace('USDT','')+'</td><td style="font-size:.82rem">'+(a.condition==='above'?'📈':'📉')+' $'+parseFloat(a.targetPrice).toFixed(4)+'</td><td style="font-size:.78rem;color:var(--text2)">'+new Date(a.createdAt).toLocaleDateString()+'</td></tr>').join('');
        res.send(_html('Price Alerts', `
${_appNav('alerts', user.username)}
<div class="wrap">
  <h1 class="page-title">🔔 Price Alerts <span>WhatsApp notification when price hits target</span></h1>
  ${noWa}
  <div class="panel" style="max-width:560px;margin-bottom:20px">
    ...
    <div class="panel-body">
      <div style="display:flex;gap:10px;flex-wrap:wrap;align-items:flex-end">
        <div style="flex:1;min-width:100px"><label class="field-label">Coin</label><input type="text" id="al-coin" class="inp" placeholder="BTC" style="font-family:var(--font-mono);font-weight:700;text-transform:uppercase" maxlength="10"></div>
        <div style="min-width:130px"><label class="field-label">Condition</label><select id="al-cond" class="inp"><option value="above">📈 Price Above</option><option value="below">📉 Price Below</option></select></div>
        <div style="min-width:120px"><label class="field-label">Target Price ($)</label><input type="number" id="al-price" class="inp" placeholder="0.00" step="any"></div>
        <button class="btn btn-primary" id="al-btn">🔔 Set Alert</button>
      </div>
      <div id="al-msg" style="margin-top:8px;font-size:.83rem"></div>
    </div>
  </div>
  <div class="panel" style="margin-bottom:20px">
    <div class="panel-head"><div class="panel-title">🔔 Active Alerts (${active.length})</div></div>
    <div class="panel-body" style="padding:0"><div class="tbl-wrap">
  ...
  <div class="panel">
    <div class="panel-head"><div class="panel-title">✅ Triggered</div></div>
    <div class="panel-body" style="padding:0"><div class="tbl-wrap">
...
<div class="toast" id="toast"></div>
<script>
function showToast(m,t){var el=document.getElementById('toast');el.textContent=m;el.className='toast'+(t==='err'?' err':'');el.style.display='block';clearTimeout(el._t);el._t=setTimeout(function(){el.style.display='none';},3000);}
document.getElementById('al-coin').addEventListener('input',function(){this.value=this.value.toUpperCase().replace(/[^A-Z0-9]/g,'');});
document.getElementById('al-btn').addEventListener('click',function(){
  var coin=document.getElementById('al-coin').value.trim();
  ...
  fetch('/app/api/alerts',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({coin:coin,condition:cond,targetPrice:price})})
  .then(function(r){return r.json();}).then(function(d){
    if(d.ok){msg.style.color='var(--green)';msg.textContent='✅ Alert created!';setTimeout(function(){location.reload();},1200);}
    else{msg.style.color='var(--red)';msg.textContent='❌ '+(d.error||'Failed');}
  }).catch(function(){msg.style.color='var(--red)';msg.textContent='❌ Network error';});
});
function delAlert(id){
  if(!confirm('Delete this alert?'))return;
  fetch('/app/api/alerts/'+id,{method:'DELETE'}).then(function(r){return r.json();}).then(function(d){
    if(d.ok){showToast('✅ Deleted');setTimeout(function(){location.reload();},600);}
    else showToast('❌ '+(d.error||'Failed'),'err');
  });
}
</script>`));
    ...
    // ════════════════════════════════════════════════════════════════════
    //  WATCHLIST PAGE  GET /app/watchlist
    // ════════════════════════════════════════════════════════════════════
    app.get('/app/watchlist', saasAuth.requireUserAuth, async (req, res) => {
        ...
        const noWa = !waJid ? '<div style="background:rgba(255,171,0,.08);border:1px solid rgba(255,171,0,.2);border-radius:10px;padding:13px 16px;margin-bottom:20px;font-size:.84rem;color:#ffab00">⚠️ WhatsApp not linked. <a href="/app/settings" style="color:#00c8ff">Link →</a></div>' : '';
        const coinCards = coins.length===0
            ? '<div style="grid-column:1/-1;text-align:center;padding:32px;color:var(--text2)"><div style="font-size:2rem;margin-bottom:8px">👁</div>No coins yet. Add above.</div>'
            : coins.map(c=>'<div class="stat-card" id="wc-'+c+'" style="cursor:pointer" onclick="analyseWL(\''+c.replace('USDT','')+'\')"><div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px"><div style="font-family:var(--font-head);font-size:1rem;font-weight:800">'+c.replace('USDT','')+'</div><button class="btn btn-danger btn-sm" style="padding:2px 7px;font-size:.65rem" onclick="event.stopPropagation();removeWL(\''+c+'\')">✕</button></div><div style="font-family:var(--font-mono);font-size:1.1rem;font-weight:700;color:var(--accent)" id="wp-'+c+'">—</div><div style="font-size:.7rem;margin-top:4px" id="wch-'+c+'">Loading...</div><div style="font-size:.65rem;color:var(--accent);margin-top:6px">Click → Analyse</div></div>').join('');
  ...
  <div class="panel" style="max-width:500px;margin-bottom:20px">
    ...
    <div class="panel-body">
      <div style="display:flex;gap:10px;align-items:flex-end">
        <div style="flex:1"><label class="field-label">Coins (space-separated)</label><input type="text" id="wl-inp" class="inp" placeholder="BTC ETH SOL AVAX" style="text-transform:uppercase;font-family:var(--font-mono)"></div>
        <button class="btn btn-primary" id="wl-add-btn">Add →</button>
      </div>
      <div id="wl-msg" style="margin-top:8px;font-size:.82rem"></div>
    </div>
  </div>
  <div class="panel">
    <div class="panel-head"><div class="panel-title">👁 My Watchlist</div><button class="btn btn-ghost btn-sm" id="wl-refresh-btn">↺ Refresh Prices</button></div>
    <div id="wl-grid" style="padding:16px 20px;display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:12px">${coinCards}</div>
  </div>
</div>
<div class="toast" id="toast"></div>
<script>
function showToast(m,t){var el=document.getElementById('toast');el.textContent=m;el.className='toast'+(t==='err'?' err':'');el.style.display='block';clearTimeout(el._t);el._t=setTimeout(function(){el.style.display='none';},3000);}
document.getElementById('wl-inp').addEventListener('input',function(){this.value=this.value.toUpperCase();});
document.getElementById('wl-add-btn').addEventListener('click',function(){
  var raw=document.getElementById('wl-inp').value.trim();
  var msg=document.getElementById('wl-msg');
  if(!raw){msg.style.color='var(--red)';msg.textContent='❌ Enter coins';return;}
  var coins=raw.split(/[\s,]+/).filter(Boolean);
  msg.style.color='var(--text2)';msg.textContent='⏳ Adding...';
  fetch('/app/api/watchlist',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({coins:coins})})
  .then(function(r){return r.json();}).then(function(d){
    if(d.ok){msg.style.color='var(--green)';msg.textContent='✅ Added!';setTimeout(function(){location.reload();},1000);}
    else{msg.style.color='var(--red)';msg.textContent='❌ '+(d.error||'Failed');}
  }).catch(function(){msg.style.color='var(--red)';msg.textContent='❌ Network error';});
});
document.getElementById('wl-refresh-btn').addEventListener('click',refreshPrices);
function removeWL(coin){
  fetch('/app/api/watchlist/'+coin,{method:'DELETE'}).then(function(r){return r.json();}).then(function(d){
    if(d.ok){showToast('✅ Removed');var el=document.getElementById('wc-'+coin);if(el)el.remove();}
    else showToast('❌ '+(d.error||'Failed'),'err');
  });
}
function analyseWL(coin){window.location.href='/app/scanner?coin='+coin;}
function refreshPrices(){
  fetch('/app/api/watchlist/prices').then(function(r){return r.json();}).then(function(d){
    if(d.ok&&d.prices){
      d.prices.forEach(function(p){
        var pe=document.getElementById('wp-'+p.coin);
        var ch=document.getElementById('wch-'+p.coin);
        if(pe&&p.price){
          var pr=parseFloat(p.price);
          pe.textContent=pr>=10000?'$'+pr.toFixed(2):pr>=1?'$'+pr.toFixed(4):'$'+pr.toFixed(6);
          pe.style.color=parseFloat(p.change||0)>=0?'var(--green)':'var(--red)';
        }
        if(ch&&p.change!=null){
          ch.textContent=(parseFloat(p.change)>=0?'+':'')+parseFloat(p.change).toFixed(2)+'% 24h';
          ch.style.color=parseFloat(p.change)>=0?'var(--green)':'var(--red)';
        }
      });
    }
  }).catch(function(){});
...
if(params.get('coin')) document.getElementById('wl-inp').value=params.get('coin');
</script>`));
    });

    // ════════════════════════════════════════════════════════════════════
    //  MARKET INTEL PAGE  GET /app/news
    // ════════════════════════════════════════════════════════════════════
    app.get('/app/news', saasAuth.requireUserAuth, (req, res) => {
        const user = req.saasUser;
...
<div class="wrap">
  ...
  <div id="intel-loading" style="padding:60px;text-align:center">
    <div style="font-size:2.5rem" id="intel-spin">📡</div>
    <div style="margin-top:12px;font-size:.9rem;color:var(--text)">Loading market data...</div>
  </div>
...
function fmtIntel(){
  var spins=['📡','📊','🌐','💹'];
  var si=0;
  var st=setInterval(function(){document.getElementById('intel-spin').textContent=spins[si++%spins.length];},700);
  fetch('/app/api/market-intel').then(function(r){return r.json();}).then(function(d){
    clearInterval(st);
    if(!d.ok){document.getElementById('intel-loading').innerHTML='<div style="color:var(--red)">❌ '+(d.error||'Failed')+'</div>';return;}
    var s=d.sent||{};
    var fv=parseInt(s.fngValue||50);
    var fc=fv<25?'var(--red)':fv<45?'var(--yellow)':fv<60?'var(--green)':fv<80?'var(--yellow)':'var(--red)';
    var ns=s.newsSentimentScore||0;
    var headlines=(s.newsHeadlines||[]).slice(0,5);
    var funding=d.topFunding||[];
    var html='<div class="g-stats" style="margin-bottom:22px">';
    html+='<div class="stat-card"><div class="stat-label">Fear & Greed</div><div style="font-size:2rem;font-family:var(--font-mono);font-weight:800;line-height:1;color:'+fc+'">'+(s.fngEmoji||'⚪')+' '+(s.fngValue||'—')+'</div><div class="stat-sub">'+(s.fngLabel||'—')+'</div></div>';
    html+='<div class="stat-card"><div class="stat-label">₿ BTC Dominance</div><div class="stat-val c-yellow">'+(s.btcDominance||'—')+'%</div><div class="stat-sub">'+(parseInt(s.btcDominance)>55?'Alts suffering':parseInt(s.btcDominance)<45?'Altseason':'Balanced')+'</div></div>';
    html+='<div class="stat-card"><div class="stat-label">News Sentiment</div><div class="stat-val" style="color:'+(ns>0?'var(--green)':ns<0?'var(--red)':'var(--text2)')+'">'+( ns>=0?'+':'')+ns+' / 5</div><div class="stat-sub">'+(ns>2?'Very Bullish':ns>0?'Bullish':ns<-2?'Very Bearish':ns<0?'Bearish':'Neutral')+'</div></div>';
    html+='<div class="stat-card"><div class="stat-label">Overall Bias</div><div style="font-size:.9rem;font-weight:700;margin-top:8px;color:var(--text)">'+(s.overallSentiment||'—')+'</div><div class="stat-sub">'+(s.tradingBias||'—')+'</div></div>';
    html+='</div>';
    if(funding.length){
      html+='<div class="panel" style="margin-bottom:20px"><div class="panel-head"><div class="panel-title">💸 Extreme Funding Rates</div><div style="font-size:.75rem;color:var(--text2)">>0.1% = squeeze risk</div></div><div class="panel-body" style="padding:0"><table style="width:100%"><thead><tr><th>Coin</th><th>Funding Rate</th><th>Signal</th></tr></thead><tbody>';
      funding.forEach(function(f){
        html+='<tr><td style="font-family:var(--font-mono);font-weight:700">'+f.coin+'</td><td style="font-family:var(--font-mono);color:'+(parseFloat(f.rate)>0?'var(--red)':'var(--green)')+';">'+(parseFloat(f.rate)>=0?'+':'')+f.rate+'%</td><td style="font-size:.82rem">'+f.signal+'</td></tr>';
      });
      html+='</tbody></table></div></div>';
    }
    if(headlines.length){
      html+='<div class="panel"><div class="panel-head"><div class="panel-title">📰 Latest News</div></div><div class="panel-body"><div style="display:flex;flex-direction:column;gap:10px">';
      headlines.forEach(function(h){html+='<div style="padding:10px 14px;background:var(--bg2);border-radius:var(--radius-sm);font-size:.84rem;border-left:3px solid var(--border2)">'+h+'</div>';});
      html+='</div></div></div>';
    }
    html+='<div style="text-align:right;font-size:.7rem;color:var(--text2);margin-top:10px">Updated at '+new Date().toLocaleTimeString()+'</div>';
  ...
  }).catch(function(e){document.getElementById('intel-loading').innerHTML='<div style="color:var(--red)">❌ '+e.message+'</div>';});
}
fmtIntel();
</script>`));
    });
    ...
    // ════════════════════════════════════════════════════════════════════
    //  RISK CALCULATOR PAGE  GET /app/calc
    // ════════════════════════════════════════════════════════════════════
...
<div class="wrap">
  <h1 class="page-title">🧮 Risk Calculator <span>Position sizing · Binance Futures</span></h1>
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;align-items:start">
    <div class="panel">
      <div class="panel-head"><div class="panel-title">⚙️ Parameters</div></div>
      <div class="panel-body">
        <div class="field"><label class="field-label">Account Balance ($)</label><input type="number" id="c-bal" class="inp" value="1000" min="1"></div>
        <div class="field"><label class="field-label">Risk Per Trade (%)</label><input type="number" id="c-risk" class="inp" value="2" min="0.1" max="100" step="0.1"><div style="font-size:.72rem;color:var(--text2);margin-top:4px">Pro traders use 1–2%</div></div>
        <div class="field"><label class="field-label">Entry Price ($)</label><input type="number" id="c-entry" class="inp" placeholder="42000" step="any"></div>
        <div class="field"><label class="field-label">Stop Loss ($)</label><input type="number" id="c-sl" class="inp" placeholder="41000" step="any"></div>
        <div class="field"><label class="field-label">Take Profit ($)</label><input type="number" id="c-tp" class="inp" placeholder="44000" step="any"></div>
        <button class="btn btn-primary" id="calc-btn" style="width:100%;justify-content:center">🧮 Calculate</button>
      </div>
    </div>
    <div>
      <div class="panel" style="margin-bottom:16px;border-color:rgba(0,200,255,.2)">
        ...
        <div class="panel-body">
          <div class="g-stats" style="grid-template-columns:1fr 1fr;margin-bottom:14px">
            <div class="stat-card"><div class="stat-label">Leverage</div><div class="stat-val c-yellow" id="r-lev">—</div></div>
            <div class="stat-card"><div class="stat-label">Margin</div><div class="stat-val c-cyan" id="r-margin">—</div></div>
          </div>
          <div style="display:flex;flex-direction:column;gap:6px" id="r-rows"></div>
        </div>
      </div>
      <div class="panel">
        <div class="panel-head"><div class="panel-title">📉 DCA Plan</div></div>
        <div class="panel-body"><div id="r-dca" style="color:var(--text2);font-size:.83rem">Enter values to calculate.</div></div>
      </div>
    </div>
  </div>
</div>
...
function doCalc(){
  var bal=parseFloat(document.getElementById('c-bal').value)||0;
  var rp=parseFloat(document.getElementById('c-risk').value)||2;
  var entry=parseFloat(document.getElementById('c-entry').value)||0;
  var sl=parseFloat(document.getElementById('c-sl').value)||0;
  var tp=parseFloat(document.getElementById('c-tp').value)||0;
  if(!bal||!entry||!sl)return;
  var riskAmt=bal*(rp/100);
  var slDist=Math.abs(entry-sl);
  var slPct=slDist/entry*100;
  var qty=slDist>0?riskAmt/slDist:0;
  var lev=Math.min(Math.ceil(slPct>0?(rp/slPct)/0.10:10),75);
  var marginUsed=qty>0?(qty*entry)/lev:0;
  var potProfit=tp>0&&qty>0?Math.abs(tp-entry)*qty:0;
  var rrr=slDist>0&&tp>0?Math.abs(tp-entry)/slDist:0;
  var isLong=tp>entry;
  ...
  var rows=[
    ['💰 Risk Amount','$'+riskAmt.toFixed(2)+' ('+rp+'%)'],
    ['📦 Position Size',qty>0?(qty<1?qty.toFixed(4):Math.round(qty))+' units':'—'],
    ['🎯 Risk/Reward',rrr>0?'1:'+rrr.toFixed(2):'Enter TP'],
    ['📉 SL Distance',slPct.toFixed(2)+'% ($'+slDist.toFixed(4)+')'],
    ['💸 Potential Profit',tp>0?'$'+potProfit.toFixed(2):'Enter TP'],
  ];
  var rEl=document.getElementById('r-rows');
  rEl.innerHTML='';
  rows.forEach(function(row){
    var d=document.createElement('div');
    d.style.cssText='display:flex;justify-content:space-between;padding:8px 10px;background:var(--bg2);border-radius:5px';
    d.innerHTML='<span style="font-size:.8rem;color:var(--text2)">'+row[0]+'</span><span style="font-family:var(--font-mono);font-size:.85rem;font-weight:600">'+row[1]+'</span>';
    rEl.appendChild(d);
  });
  var dca1=isLong?entry-slDist*0.35:entry+slDist*0.35;
  var dca2=isLong?entry-slDist*0.70:entry+slDist*0.70;
  var dcaData=[['Entry (50%)',entry,'var(--accent)'],['DCA 1 — 35% to SL',dca1,'var(--yellow)'],['DCA 2 — 70% to SL',dca2,'var(--red)'],['Hard SL',sl,'var(--red)']];
  var dcaEl=document.getElementById('r-dca');
  dcaEl.innerHTML='';
  dcaData.forEach(function(item){
    var d=document.createElement('div');
    d.style.cssText='display:flex;justify-content:space-between;padding:6px 8px;background:var(--bg2);border-radius:5px;margin-bottom:5px';
    d.innerHTML='<span style="font-size:.78rem;color:var(--text2)">'+item[0]+'</span><span style="font-family:var(--font-mono);font-size:.82rem;font-weight:600;color:'+item[2]+'">'+fmtP2(item[1])+'</span>';
    dcaEl.appendChild(d);
  });
}
document.getElementById('calc-btn').addEventListener('click',doCalc);
['c-bal','c-risk','c-entry','c-sl','c-tp'].forEach(function(id){
  document.getElementById(id).addEventListener('keyup',doCalc);
});
</script>`));
    });

    // ════════════════════════════════════════════════════════════════════
    //  ENHANCED SCANNER PAGE (replaces old one) — 4 tabs
    // ════════════════════════════════════════════════════════════════════
...
<div class="wrap">
  <h1 class="page-title">⚡ AI Scanner <span>70-Factor Analysis · .future Parity</span></h1>
  <div style="display:flex;gap:0;border:1px solid var(--border);border-radius:var(--radius);overflow:hidden;margin-bottom:22px;width:fit-content">
    <button id="tab-live" style="padding:9px 20px;font-size:.82rem;font-weight:600;border:none;cursor:pointer;background:var(--accent);color:#000;font-family:var(--font-mono)">⚡ Live Analysis</button>
    <button id="tab-bt"   style="padding:9px 20px;font-size:.82rem;font-weight:600;border:none;cursor:pointer;background:var(--card2);color:var(--text2);font-family:var(--font-mono)">📊 Backtest</button>
    <button id="tab-sbt"  style="padding:9px 20px;font-size:.82rem;font-weight:600;border:none;cursor:pointer;background:var(--card2);color:var(--text2);font-family:var(--font-mono)">🌐 Scan Backtest</button>
    <button id="tab-ms"   style="padding:9px 20px;font-size:.82rem;font-weight:600;border:none;cursor:pointer;background:var(--card2);color:var(--text2);font-family:var(--font-mono)">🔍 Market Scan</button>
  </div>
    ...
    <div class="panel" style="max-width:680px;margin-bottom:22px">
      <div class="panel-body">
        <div style="display:flex;gap:12px;flex-wrap:wrap;align-items:flex-end">
          <div style="flex:1;min-width:140px"><label class="field-label">Coin</label>
            <input type="text" id="coin-inp" class="inp" placeholder="BTC, ETH, SOL..." style="font-family:var(--font-mono);font-size:1.05rem;font-weight:700;text-transform:uppercase" maxlength="12" autocomplete="off"></div>
          <div style="min-width:130px"><label class="field-label">Timeframe</label>
            <select id="tf-sel" class="inp"><option value="5m">5m</option><option value="15m" selected>15m</option><option value="1h">1h</option><option value="4h">4h</option><option value="1d">1d</option></select></div>
          <button class="btn btn-primary" style="padding:11px 28px" id="scan-btn">⚡ Analyse</button>
        </div>
        <div style="font-size:.72rem;color:var(--text2);margin-top:8px">70-Factor MTF · SMC · Funding Rate · Whale Walls · 11-Factor Confirmation · AI</div>
      </div>
    </div>
  ...
  <div id="panel-bt" style="display:none">
    <div class="panel" style="max-width:600px;margin-bottom:22px">
      <div class="panel-body">
        <div style="display:flex;gap:12px;flex-wrap:wrap;align-items:flex-end">
          <div style="flex:1;min-width:140px"><label class="field-label">Coin</label>
            <input type="text" id="bt-coin" class="inp" placeholder="BTC, ETH..." style="font-family:var(--font-mono);font-size:1.05rem;font-weight:700;text-transform:uppercase" maxlength="12"></div>
          <div style="min-width:130px"><label class="field-label">Timeframe</label>
            <select id="bt-tf" class="inp"><option value="5m">5m</option><option value="15m" selected>15m</option><option value="1h">1h</option><option value="4h">4h</option><option value="1d">1d</option></select></div>
          <button class="btn btn-warn" style="padding:11px 22px" id="bt-btn">📊 Run Backtest</button>
        </div>
        <div style="font-size:.72rem;color:var(--text2);margin-top:8px">1000 candles · TP1/TP2/TP3 partial close simulation</div>
      </div>
    </div>
    <div id="bt-loading" style="display:none;padding:50px;text-align:center"><div style="font-size:2.5rem;display:inline-block" id="bt-spin">📊</div><div style="margin-top:14px;font-size:.95rem;color:var(--text)" id="bt-msg">Running...</div></div>
    <div id="bt-error" style="display:none;background:rgba(255,51,85,.08);border:1px solid rgba(255,51,85,.25);border-radius:var(--radius);padding:14px 18px;margin-bottom:16px;color:var(--red);font-size:.88rem"></div>
    <div id="bt-result" style="display:none"></div>
  </div>

  <!-- SCAN BACKTEST TAB -->
  <div id="panel-sbt" style="display:none">
    <div class="panel" style="max-width:500px;margin-bottom:22px">
      <div class="panel-body">
        <div style="display:flex;gap:12px;flex-wrap:wrap;align-items:flex-end">
          <div style="min-width:130px"><label class="field-label">Timeframe</label>
            <select id="sbt-tf" class="inp"><option value="15m" selected>15m</option><option value="1h">1h</option><option value="4h">4h</option></select></div>
          <div style="min-width:110px"><label class="field-label">Coins</label>
            <select id="sbt-limit" class="inp"><option value="10">Top 10</option><option value="15" selected>Top 15</option><option value="20">Top 20</option></select></div>
          <button class="btn btn-primary" style="padding:11px 22px" id="sbt-btn">🌐 Scan All</button>
        </div>
        <div style="font-size:.72rem;color:var(--text2);margin-top:8px">Backtest top coins · Rank by Win Rate · Find best pairs</div>
      </div>
    </div>
    <div id="sbt-loading" style="display:none;padding:50px;text-align:center"><div style="font-size:2.5rem;display:inline-block" id="sbt-spin">🌐</div><div style="margin-top:14px;font-size:.95rem;color:var(--text)" id="sbt-msg">Scanning coins...</div></div>
    <div id="sbt-error" style="display:none;background:rgba(255,51,85,.08);border:1px solid rgba(255,51,85,.25);border-radius:var(--radius);padding:14px 18px;margin-bottom:16px;color:var(--red);font-size:.88rem"></div>
    <div id="sbt-result" style="display:none"></div>
  </div>
  ...
  <div id="panel-ms" style="display:none">
    <div class="panel" style="margin-bottom:22px;border-color:rgba(0,200,255,.2)">
      <div class="panel-head" style="justify-content:space-between;flex-wrap:wrap;gap:10px">
        <div class="panel-title">🌐 Auto Market Scanner</div>
        <button class="btn btn-primary" id="ms-btn">⚡ Scan Now</button>
      </div>
      <div id="ms-loading" style="display:none;padding:40px;text-align:center"><div style="font-size:2.2rem;display:inline-block" id="ms-spin">🌐</div><div style="margin-top:12px;font-size:.9rem;color:var(--text)">Scanning top coins...</div></div>
      <div id="ms-error" style="display:none;padding:14px 20px;color:var(--red);font-size:.88rem"></div>
      <div id="ms-empty" style="display:none;padding:32px 20px;text-align:center;color:var(--text2)"><div style="font-size:2rem;margin-bottom:8px">🔍</div>No high-quality setups right now.</div>
      <div id="ms-result" style="display:none;padding:16px 20px"><div id="ms-meta" style="font-size:.75rem;color:var(--text2);margin-bottom:12px"></div><div id="ms-grid" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(240px,1fr));gap:12px"></div></div>
    </div>
  </div>
</div>
...
.ci{background:var(--bg2);border-radius:5px;padding:6px 8px;font-size:.7rem}
.ci-l{color:var(--text2);margin-bottom:2px}
.ci-v{font-family:var(--font-mono);font-size:.68rem;color:var(--text)}
.opp-card{border-radius:var(--radius);border:1px solid var(--border);padding:14px;background:var(--card);cursor:pointer;transition:.15s}
.opp-card:hover{transform:translateY(-2px);border-color:var(--border2)}
.opp-card.lc{border-left:3px solid var(--green)}.opp-card.sc{border-left:3px solid var(--red)}
.sbt-row{display:flex;align-items:center;gap:12px;padding:12px 16px;border-bottom:1px solid var(--border);transition:.1s}
.sbt-row:last-child{border-bottom:none}.sbt-row:hover{background:rgba(0,200,255,.03)}
</style>
...
function switchTab(t){
  _tab=t;
  var tabs=['live','bt','sbt','ms'];
  tabs.forEach(function(id){
    var tb=document.getElementById('tab-'+id);
    var pn=document.getElementById('panel-'+id);
    tb.style.background=t===id?'var(--accent)':'var(--card2)';
    tb.style.color=t===id?'#000':'var(--text2)';
    pn.style.display=t===id?'':'none';
  });
}

document.getElementById('tab-live').addEventListener('click',function(){switchTab('live');});
document.getElementById('tab-bt').addEventListener('click',function(){switchTab('bt');});
document.getElementById('tab-sbt').addEventListener('click',function(){switchTab('sbt');});
document.getElementById('tab-ms').addEventListener('click',function(){switchTab('ms');});
...
// ── LIVE ANALYSIS ─────────────────────────────────────────────────
document.getElementById('coin-inp').addEventListener('input',function(){this.value=this.value.toUpperCase().replace(/[^A-Z0-9]/g,'');});
document.getElementById('coin-inp').addEventListener('keydown',function(e){if(e.key==='Enter')doScan();});
document.getElementById('scan-btn').addEventListener('click',doScan);

function doScan(){
  var coin=document.getElementById('coin-inp').value.trim();
  if(!coin){document.getElementById('coin-inp').focus();return;}
  document.getElementById('scan-result').style.display='none';
  document.getElementById('scan-error').style.display='none';
  document.getElementById('scan-loading').style.display='';
  document.getElementById('scan-btn').disabled=true;
  var msgs=['Running analysis...','Calculating indicators...','Fetching funding & whale data...','11-Factor confirmation...','AI processing...','Almost done...'];
  var mi=0;
  var mt=setInterval(function(){document.getElementById('scan-msg').textContent=msgs[Math.min(++mi,msgs.length-1)];},4000);
  var spinF=['⚙️','🔄','⚡','🔍'];var si=0;
  var st=setInterval(function(){document.getElementById('spin-icon').textContent=spinF[si++%spinF.length];},600);
  fetch('/app/api/scan',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({coin:coin,timeframe:document.getElementById('tf-sel').value})})
  .then(function(r){return r.json();}).then(function(d){
    clearInterval(mt);clearInterval(st);
    document.getElementById('scan-loading').style.display='none';
    if(!d.ok){document.getElementById('scan-error').style.display='';document.getElementById('scan-error').textContent='❌ '+(d.error||'Analysis failed');return;}
    renderScan(d.analysis,coin);
  }).catch(function(e){
    clearInterval(mt);clearInterval(st);
    document.getElementById('scan-loading').style.display='none';
    document.getElementById('scan-error').style.display='';document.getElementById('scan-error').textContent='❌ Network error: '+e.message;
  }).finally(function(){document.getElementById('scan-btn').disabled=false;});
}

function renderScan(a,coin){
  var isL=a.direction==='LONG',dc=isL?'var(--green)':'var(--red)',da=isL?'▲ LONG':'▼ SHORT';
  var sc=sCol(a.score||0),f=a.futures||{},s=a.sentiment||{},ec=a.entryConf||{},ai=a.ai||{};
  var entry=parseFloat(a.entryPrice)||0,sl=parseFloat(a.sl)||0,risk=Math.abs(entry-sl);
  ...
  // Hero
  h+='<div class="rcard" style="background:linear-gradient(135deg,#031526,#060d17);border-color:'+sc+'">';
  h+='<div style="display:flex;align-items:center;gap:16px;flex-wrap:wrap">';
  h+='<div style="width:80px;height:80px;border-radius:50%;border:3px solid '+sc+';display:flex;align-items:center;justify-content:center;font-family:var(--font-mono);font-size:1.6rem;font-weight:800;color:'+sc+';flex-shrink:0">'+(a.score||0)+'</div>';
  h+='<div style="flex:1"><div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin-bottom:8px">';
  h+='<span style="font-family:var(--font-head);font-size:1.4rem;font-weight:800;color:#fff">'+coin.replace('USDT','')+'/USDT</span>';
  h+='<span style="padding:4px 14px;border-radius:99px;font-size:.85rem;font-weight:700;background:'+(isL?'rgba(0,230,118,.12)':'rgba(255,51,85,.12)')+';color:'+dc+'">'+da+'</span>';
  h+='<span style="font-family:var(--font-mono);font-size:.82rem;color:var(--text2)">'+fmtP(a.currentPrice)+'</span>';
  h+='</div><div style="font-size:.72rem;color:var(--text2);line-height:1.8">';
  reasons.slice(0,8).forEach(function(r){h+='<span style="display:inline-block;background:rgba(0,200,255,.07);border-radius:4px;padding:1px 6px;margin:1px">'+r+'</span>';});
  h+='</div><div style="margin-top:8px;display:flex;gap:14px;flex-wrap:wrap;font-size:.75rem;color:var(--text2)">';
  h+='<span>ADX: <b style="color:var(--text)">'+(a.adxData?parseFloat(a.adxData.value||0).toFixed(1)+' · '+a.adxData.status:'—')+'</b></span>';
  h+='<span>Market: <b style="color:var(--text)">'+(a.marketState||'—')+'</b></span>';
  h+='<span>AI: <b style="color:var(--accent)">'+(ai.confidence||'—')+'</b></span>';
  h+='<span>Gate: <b style="color:'+(a.confGate?'var(--green)':'var(--text2)')+'">'+(a.confGate?'✅ PASSED':''+( a.confScore||0)+'/14')+'</b></span>';
  h+='</div></div></div></div>';

  // Trade Setup
  h+='<div class="rcard" style="border-color:rgba(255,171,0,.2)"><div class="rcard-title">⚡ Futures Setup</div>';
  h+='<div class="rgrid" style="margin-bottom:12px">';
  h+='<div class="ritem"><div class="ritem-label">Leverage</div><div class="ritem-val" style="color:var(--yellow);font-size:1.1rem">Cross '+(f.leverage||'—')+'x</div></div>';
  h+='<div class="ritem"><div class="ritem-label">Margin</div><div class="ritem-val" style="color:var(--yellow)">2% wallet</div></div>';
  h+='<div class="ritem"><div class="ritem-label">Entry</div><div class="ritem-val" style="color:var(--accent)">'+fmtP(a.entryPrice)+'</div></div>';
  h+='<div class="ritem"><div class="ritem-label">Stop Loss</div><div class="ritem-val" style="color:var(--red)">'+fmtP(a.sl)+'</div></div>';
  h+='<div class="ritem"><div class="ritem-label">RRR</div><div class="ritem-val" style="color:'+(parseFloat(f.rrr||0)>=2?'var(--green)':parseFloat(f.rrr||0)>=1?'var(--yellow)':'var(--red)')+'">1:'+(f.rrr||a.rrr||'—')+'</div></div>';
  h+='</div>';
  h+='<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:12px">';
  h+='<div class="ritem"><div class="ritem-label">TP1 — 33%</div><div class="ritem-val" style="color:var(--green)">'+fmtP(a.tp1)+'</div></div>';
  h+='<div class="ritem"><div class="ritem-label">TP2 — 33%</div><div class="ritem-val" style="color:var(--green)">'+fmtP(a.tp2)+'</div></div>';
  h+='<div class="ritem"><div class="ritem-label">TP3 — 34%</div><div class="ritem-val" style="color:var(--green)">'+fmtP(a.tp3)+'</div></div>';
  h+='</div>';
  h+='<div style="background:rgba(0,200,255,.04);border:1px solid rgba(0,200,255,.1);border-radius:8px;padding:10px 12px"><div style="font-size:.64rem;font-weight:700;color:var(--accent);text-transform:uppercase;letter-spacing:.07em;margin-bottom:8px">DCA Points</div>';
  h+='<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:6px">';
  [['Entry',fmtP(a.entryPrice),'var(--text)'],['DCA 1 (35% to SL)',fmtP(dca1),'var(--yellow)'],['DCA 2 (70% to SL)',fmtP(dca2),'var(--red)'],['Hard SL',fmtP(a.sl),'var(--red)']].forEach(function(row){
    h+='<div><div style="font-size:.6rem;color:var(--text2)">'+row[0]+'</div><div style="font-family:var(--font-mono);font-size:.82rem;color:'+row[2]+'">'+row[1]+'</div></div>';
  });
  h+='</div></div></div>';

  // Market + Sentiment (2 col)
  h+='<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:12px">';
  h+='<div class="rcard" style="margin-bottom:0"><div class="rcard-title">📊 Key Indicators</div>';
  [['RSI',a.rsi?parseFloat(a.rsi).toFixed(1):'—'],['VWAP',a.vwap?String(a.vwap).replace(/🟢|🔴/g,'').trim():'—'],['4H Trend',a.trend4H||'—'],['1H Trend',a.trend1H||'—'],['Wyckoff',a.wyckoff?(a.wyckoff.phase||'—'):'—'],['SuperTrend',a.supertrend?(a.supertrend.signal||'—'):'—']].forEach(function(row){
    h+='<div class="rrow"><span style="color:var(--text2)">'+row[0]+'</span><span style="font-family:var(--font-mono);font-size:.8rem">'+row[1]+'</span></div>';
  });
  h+='</div>';
  h+='<div class="rcard" style="margin-bottom:0"><div class="rcard-title">🌊 Market Data</div>';
  [['F&G',(s.fngEmoji||'⚪')+' '+(s.fngValue||'—')+' ('+( s.fngLabel||'—')+')'],['BTC Dom',(s.btcDom||'—')+'%'],['News',(s.newsScore>=0?'+':'')+( s.newsScore||0)],['Funding',a.fundingRate||'N/A'],['Buy Wall',a.whaleWalls?fmtP(a.whaleWalls.supportWall):'—'],['Sell Wall',a.whaleWalls?fmtP(a.whaleWalls.resistWall):'—']].forEach(function(row){
    h+='<div class="rrow"><span style="color:var(--text2)">'+row[0]+'</span><span style="font-family:var(--font-mono);font-size:.8rem">'+row[1]+'</span></div>';
  });
  h+='</div></div>';

  // AI
  h+='<div class="rcard" style="border-color:rgba(124,58,237,.2);background:rgba(124,58,237,.03)"><div class="rcard-title">🤖 AI Analysis · '+(ai.confidence||'—')+' Confidence</div>';
  h+='<div style="font-size:.85rem;color:var(--text);line-height:1.7">'+(ai.trend||'—')+'</div>';
  if(ai.smc_summary)h+='<div style="font-size:.8rem;color:var(--text2);margin-top:6px">'+ai.smc_summary+'</div>';
  h+='</div>';

  // 11-Factor
  var factors=ec.factors||{};
  var fl=[['Stablecoin',factors.usdtDom],['OI',factors.oiChange],['CVD',factors.cvd],['L/S',factors.lsRatio],['Funding',factors.fundingM],['OrderBook',factors.orderBook],['Whales',factors.whaleActivity],['BTC Corr',factors.btcCorr],['HTF',factors.htfLevels],['P/C Ratio',factors.pcr],['Netflow',factors.netflow],['Social',factors.social]];
  h+='<div class="rcard" style="border-color:rgba(0,200,255,.15)"><div class="rcard-title" style="display:flex;justify-content:space-between">🔬 11-Factor Confirmation <span style="color:'+(ec.strength==='STRONG'?'var(--green)':ec.strength==='MODERATE'?'var(--accent)':ec.strength==='CONFLICT'?'var(--red)':'var(--text2)')+'">'+( ec.verdict||'—')+'</span></div>';
  h+='<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:4px;margin-bottom:10px">';
  fl.forEach(function(item){
    var lbl=item[0],f2=item[1]||{};
    var sig=f2.signal||'N/A';
    var bg=sig==='BULL'||sig==='POSITIVE'?'rgba(0,230,118,.07)':sig==='BEAR'||sig==='NEGATIVE'?'rgba(255,51,85,.07)':'rgba(0,0,0,.15)';
    h+='<div class="ci" style="background:'+bg+'"><div class="ci-l">'+(f2.emoji||'⚪')+' '+lbl+'</div><div class="ci-v">'+((f2.display||'N/A').slice(0,45))+'</div></div>';
  });
  h+='</div><div style="display:flex;justify-content:space-between;padding-top:8px;border-top:1px solid var(--border);font-size:.78rem">';
  h+='<span style="color:var(--text2)">'+(ec.verdictDetail||'')+'</span>';
  h+='<span style="font-family:var(--font-mono);color:'+(ec.strength==='STRONG'?'var(--green)':ec.strength==='MODERATE'?'var(--accent)':'var(--text2)')+'">Score: '+(ec.totalScore>=0?'+':'')+parseFloat(ec.totalScore||0).toFixed(1)+' ('+( ec.strength||'WEAK')+')</span></div></div>';

  // 14-Gate
  var checks=a.confChecks||{};
  var gi=[['HTF Aligned','htfAligned'],['ChoCH','chochPrimary'],['Sweep','sweepPrimary'],['Volume','volumeConf'],['Wyckoff','wyckoffConf'],['Ichimoku','ichimokuConf'],['Supertrend','supTrendConf'],['Fib Zone','fibZoneConf'],['BB Explode','bbExplosion'],['MM Trap','mmTrapConf'],['BOS','bosConf'],['Daily Gate','dailyGate'],['AI Model','aiConf'],['Bybit','bybitConf']];
  h+='<div class="rcard"><div class="rcard-title" style="display:flex;justify-content:space-between">✅ 14-Factor Gate <span style="color:'+(a.confGate?'var(--green)':'var(--text2)')+'">'+(a.confGate?'PASSED':''+( a.confScore||0)+'/14')+'</span></div>';
  h+='<div style="display:grid;grid-template-columns:1fr 1fr;gap:3px">';
  gi.forEach(function(item){
    var pass=checks[item[1]];
    h+='<div style="font-size:.72rem;padding:3px 0;color:'+(pass?'var(--green)':'var(--text2)')+'"><span>'+(pass?'✅':'○')+'</span> '+item[0]+'</div>';
  });
  h+='</div></div>';

  h+='<div style="text-align:center;font-size:.7rem;color:var(--text2);margin-top:6px">⏱️ '+new Date().toLocaleString()+' · v7 PRO · Binance Futures</div>';
  document.getElementById('scan-result').innerHTML=h;
  document.getElementById('scan-result').style.display='';
}

// ── BACKTEST ──────────────────────────────────────────────────────
document.getElementById('bt-coin').addEventListener('input',function(){this.value=this.value.toUpperCase().replace(/[^A-Z0-9]/g,'');});
document.getElementById('bt-coin').addEventListener('keydown',function(e){if(e.key==='Enter')doBT();});
document.getElementById('bt-btn').addEventListener('click',doBT);

function doBT(){
  var coin=document.getElementById('bt-coin').value.trim();
  ...
  var msgs=['Downloading candles...','Replaying indicators...','Simulating exits...','Calculating stats...'];
  var mi=0;var mt=setInterval(function(){document.getElementById('bt-msg').textContent=msgs[Math.min(++mi,msgs.length-1)];},3500);
  var sf=['📊','🔄','📈','⚡'];var si=0;var st=setInterval(function(){document.getElementById('bt-spin').textContent=sf[si++%sf.length];},700);
  fetch('/app/api/backtest',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({coin:coin,timeframe:document.getElementById('bt-tf').value})})
  .then(function(r){return r.json();}).then(function(d){
    clearInterval(mt);clearInterval(st);
    document.getElementById('bt-loading').style.display='none';
    if(!d.ok){document.getElementById('bt-error').style.display='';document.getElementById('bt-error').textContent='❌ '+(d.error||'Failed');return;}
    renderBT(d.result,coin);
  }).catch(function(e){
    clearInterval(mt);clearInterval(st);
    document.getElementById('bt-loading').style.display='none';
    document.getElementById('bt-error').style.display='';document.getElementById('bt-error').textContent='❌ '+e.message;
  }).finally(function(){document.getElementById('bt-btn').disabled=false;});
}

function renderBT(r,coin){
  ...
  var verdict=pf>=2?'Highly tradeable. Strong edge.':pf>=1.5?'Solid edge. Use 2% risk.':pf>=1?'Slight edge. Use strict filters.':'Avoid this pair.';
  var tpD=r.tpBreakdown||{},tot=r.total||1;
  var h='';
  h+='<div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;margin-bottom:14px"><div><div style="font-family:var(--font-head);font-size:1.2rem;font-weight:800;color:#fff">'+coin.replace('USDT','')+' Backtest</div><div style="font-size:.75rem;color:var(--text2)">'+(r.candleCount||1000)+' candles</div></div><div style="padding:4px 14px;border-radius:99px;font-size:.8rem;font-weight:700;color:'+pfc+';border:1px solid '+pfc+'">'+grade+'</div></div>';
  h+='<div class="rcard"><div style="display:flex;align-items:center;gap:18px;flex-wrap:wrap"><div style="text-align:center;min-width:110px"><div style="font-size:.62rem;color:var(--text2);text-transform:uppercase;margin-bottom:4px">Win Rate</div><div style="font-family:var(--font-mono);font-size:3rem;font-weight:800;color:'+wrc+'">'+wr.toFixed(1)+'%</div><div style="font-size:.75rem;color:var(--text2)">'+r.wins+' W / '+r.losses+' L</div></div>';
  h+='<div style="flex:1"><div style="height:10px;background:rgba(255,51,85,.15);border-radius:99px;overflow:hidden;margin-bottom:12px"><div style="height:100%;width:'+Math.min(wr,100)+'%;background:linear-gradient(90deg,var(--green2),var(--green));border-radius:99px"></div></div>';
  h+='<div class="rgrid">';
  [['Total',r.total,'var(--text)'],['TP Hits',r.wins,'var(--green)'],['SL Hits',r.losses,'var(--red)'],['Net P&L',(net>=0?'+':'')+net.toFixed(1)+'R',net>=0?'var(--green)':'var(--red)']].forEach(function(item){
    h+='<div class="ritem"><div class="ritem-label">'+item[0]+'</div><div class="ritem-val" style="color:'+item[2]+'">'+item[1]+'</div></div>';
  });
  h+='</div></div></div></div>';
  h+='<div class="rgrid" style="margin-bottom:12px">';
  [['Profit Factor',r.pf==='∞'?'∞ 🏆':parseFloat(r.pf).toFixed(2),pfc],['Max Drawdown',r.maxDD+'R','var(--red)'],['Max Consec L',r.maxCL,'var(--red)']].forEach(function(item){
    h+='<div class="ritem"><div class="ritem-label">'+item[0]+'</div><div class="ritem-val" style="color:'+item[2]+';font-size:1.1rem">'+item[1]+'</div></div>';
  });
  h+='</div>';
  h+='<div class="rcard"><div class="rcard-title">🎯 TP Breakdown</div>';
  [['TP1 (1.5R)',tpD.tp1||0,'var(--accent)'],['TP2 (3R)',tpD.tp2||0,'var(--green2)'],['TP3 (5R)',tpD.tp3||0,'var(--green)'],['SL Hits',r.losses,'var(--red)']].forEach(function(item){
    var pct=(item[1]/tot*100);
    h+='<div style="margin-bottom:7px"><div style="display:flex;justify-content:space-between;margin-bottom:2px"><span style="font-size:.72rem;color:var(--text2)">'+item[0]+'</span><span style="font-family:var(--font-mono);font-size:.72rem">'+item[1]+' ('+pct.toFixed(1)+'%)</span></div><div style="height:6px;background:var(--border);border-radius:99px;overflow:hidden"><div style="height:100%;width:'+pct+'%;background:'+item[2]+';border-radius:99px"></div></div></div>';
  });
  h+='</div>';
  h+='<div class="rcard"><div class="rcard-title">📋 Verdict</div><div style="font-size:.85rem;color:var(--text)">'+verdict+'</div><div style="margin-top:8px;font-size:.75rem;color:'+pfc+';font-weight:600">'+grade+'</div></div>';
  document.getElementById('bt-result').innerHTML=h;document.getElementById('bt-result').style.display='';
}

// ── SCAN BACKTEST ─────────────────────────────────────────────────
document.getElementById('sbt-btn').addEventListener('click',doSBT);

function doSBT(){
  document.getElementById('sbt-result').style.display='none';
  document.getElementById('sbt-error').style.display='none';
  document.getElementById('sbt-loading').style.display='';
  document.getElementById('sbt-btn').disabled=true;
  var sf=['🌐','📊','🔄','⚡'];var si=0;var st=setInterval(function(){document.getElementById('sbt-spin').textContent=sf[si++%sf.length];},700);
  var msgs=['Fetching candles...','Running backtests...','Ranking coins...'];
  var mi=0;var mt=setInterval(function(){document.getElementById('sbt-msg').textContent=msgs[Math.min(++mi,msgs.length-1)];},4000);
  fetch('/app/api/scanbacktest',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({timeframe:document.getElementById('sbt-tf').value,limit:document.getElementById('sbt-limit').value})})
  .then(function(r){return r.json();}).then(function(d){
    clearInterval(mt);clearInterval(st);
    document.getElementById('sbt-loading').style.display='none';
    if(!d.ok){document.getElementById('sbt-error').style.display='';document.getElementById('sbt-error').textContent='❌ '+(d.error||'Failed');return;}
    renderSBT(d);
  }).catch(function(e){
    clearInterval(mt);clearInterval(st);
    document.getElementById('sbt-loading').style.display='none';
    document.getElementById('sbt-error').style.display='';document.getElementById('sbt-error').textContent='❌ '+e.message;
  }).finally(function(){document.getElementById('sbt-btn').disabled=false;});
}

function renderSBT(d){
  var rows=d.results.filter(function(r){return !r.error;});
  var h='<div class="panel"><div class="panel-head"><div class="panel-title">🌐 Scan Backtest — '+(d.timeframe||'15m').toUpperCase()+' · '+d.scanned+' coins</div></div><div>';
  rows.forEach(function(r,i){
    var wr=parseFloat(r.winRate)||0;
    var wrc=wr>=60?'var(--green)':wr>=50?'var(--accent)':wr>=40?'var(--yellow)':'var(--red)';
    h+='<div class="sbt-row">';
    h+='<div style="font-family:var(--font-mono);font-size:.72rem;color:var(--text2);min-width:22px;text-align:center">'+(i+1)+'</div>';
    h+='<div style="min-width:55px"><div style="font-family:var(--font-head);font-size:.95rem;font-weight:700;color:#fff">'+r.coin+'</div></div>';
    h+='<div style="flex:1"><div style="font-size:.68rem;color:var(--text2);margin-bottom:3px">Win Rate</div><div style="display:flex;align-items:center;gap:8px"><div style="height:6px;background:var(--border);border-radius:99px;min-width:80px;overflow:hidden"><div style="height:100%;width:'+Math.min(wr,100)+'%;background:'+wrc+';border-radius:99px"></div></div><span style="font-family:var(--font-mono);font-size:.82rem;font-weight:700;color:'+wrc+'">'+wr.toFixed(1)+'%</span></div></div>';
    h+='<div style="min-width:45px;text-align:center"><div style="font-size:.62rem;color:var(--text2)">PF</div><div style="font-family:var(--font-mono);font-size:.82rem;font-weight:700;color:'+r.gradeColor+'">'+r.pf+'</div></div>';
    h+='<div style="min-width:50px;text-align:center"><div style="font-size:.62rem;color:var(--text2)">Trades</div><div style="font-family:var(--font-mono);font-size:.82rem">'+r.total+'</div></div>';
    h+='<div style="min-width:80px;text-align:center"><span style="padding:3px 8px;border-radius:99px;font-size:.68rem;font-weight:700;color:'+r.gradeColor+';border:1px solid '+r.gradeColor+'">'+r.grade+'</span></div>';
    h+=r.recommended?'<button class="btn btn-primary btn-sm" onclick="loadScan(\''+r.coin+'\')">⚡ Analyse</button>':'<span style="font-size:.72rem;color:var(--text2)">Skip</span>';
    h+='</div>';
  });
  h+='</div></div>';
  document.getElementById('sbt-result').innerHTML=h;document.getElementById('sbt-result').style.display='';
}

function loadScan(coin){
  document.getElementById('coin-inp').value=coin;
  switchTab('live');
  setTimeout(doScan,100);
}

// ── MARKET SCAN ───────────────────────────────────────────────────
document.getElementById('ms-btn').addEventListener('click',doMS);

function doMS(){
  document.getElementById('ms-result').style.display='none';
  document.getElementById('ms-empty').style.display='none';
  document.getElementById('ms-error').style.display='none';
  document.getElementById('ms-loading').style.display='';
  document.getElementById('ms-btn').disabled=true;
  var sf=['🌐','📡','🔍','⚡'];var si=0;var st=setInterval(function(){document.getElementById('ms-spin').textContent=sf[si++%sf.length];},700);
  fetch('/app/api/market-scan')
  .then(function(r){return r.json();}).then(function(d){
    clearInterval(st);document.getElementById('ms-loading').style.display='none';
    if(!d.ok){document.getElementById('ms-error').style.display='';document.getElementById('ms-error').textContent='❌ '+(d.error||'Failed');return;}
    if(!d.setups||d.setups.length===0){document.getElementById('ms-empty').style.display='';return;}
    document.getElementById('ms-meta').textContent='Top '+d.setups.length+' from '+d.scanned+' coins';
    var g=document.getElementById('ms-grid');g.innerHTML='';
    d.setups.forEach(function(s,i){
      var isL=s.direction==='LONG',dc=isL?'var(--green)':'var(--red)';
      var sc=sCol(s.score||0),sPct=Math.min((s.score/(s.maxScore||100))*100,100);
      var div=document.createElement('div');
      div.className='opp-card '+(isL?'lc':'sc');
      div.onclick=function(){document.getElementById('coin-inp').value=s.coin;switchTab('live');setTimeout(doScan,100);};
      div.innerHTML='<div style="position:absolute;top:8px;right:10px;font-family:var(--font-mono);font-size:.6rem;color:var(--text2)">#'+(i+1)+'</div>'
        +'<div style="display:flex;align-items:center;gap:8px;margin-bottom:4px">'
        +'<div style="font-family:var(--font-head);font-size:1.05rem;font-weight:800;color:#fff">'+s.coin+'</div>'
        ...
        +'<div style="margin-left:auto;font-family:var(--font-mono);font-size:.72rem;color:'+sc+';font-weight:700">'+s.score+'/'+(s.maxScore||100)+'</div></div>'
        +'<div style="height:3px;background:var(--border);border-radius:99px;margin:6px 0;overflow:hidden"><div style="height:100%;width:'+sPct+'%;background:'+sc+';border-radius:99px"></div></div>'
        +'<div style="display:grid;grid-template-columns:1fr 1fr;gap:5px;margin-top:8px">'
        +'<div><div style="font-size:.58rem;color:var(--text2)">Entry</div><div style="font-family:var(--font-mono);font-size:.78rem;color:var(--accent)">'+fmtP(s.entryPrice)+'</div></div>'
        +'<div><div style="font-size:.58rem;color:var(--text2)">Leverage</div><div style="font-family:var(--font-mono);font-size:.78rem;color:var(--yellow)">Cross '+(s.leverage||'—')+'x</div></div>'
        +'<div><div style="font-size:.58rem;color:var(--text2)">SL</div><div style="font-family:var(--font-mono);font-size:.78rem;color:var(--red)">'+fmtP(s.sl)+'</div></div>'
        +'<div><div style="font-size:.58rem;color:var(--text2)">RRR</div><div style="font-family:var(--font-mono);font-size:.78rem;color:'+(parseFloat(s.rrr)>=2?'var(--green)':'var(--yellow)')+'">1:'+s.rrr+'</div></div>'
        +'</div>'
        +'<div style="margin-top:7px;font-size:.62rem;color:var(--accent)">Click → deep analyse →</div>';
      g.appendChild(div);
    });
    document.getElementById('ms-result').style.display='';
  }).catch(function(e){
    clearInterval(st);document.getElementById('ms-loading').style.display='none';
    document.getElementById('ms-error').style.display='';document.getElementById('ms-error').textContent='❌ '+e.message;
  }).finally(function(){document.getElementById('ms-btn').disabled=false;});
}

// Init
var initParams=new URLSearchParams(location.search);
if(initParams.get('coin')){document.getElementById('coin-inp').value=initParams.get('coin').toUpperCase();setTimeout(doScan,300);}
</script>`));
    });
