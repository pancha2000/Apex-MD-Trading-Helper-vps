...
// ── Adaptive Weight Cache (loaded from DB weekly) ────────────────────────
// Populated by loadAdaptiveWeights(). Falls back to 1.0 if DB unavailable.
let _adaptiveCache = {};  // { indicatorName: multiplier }
let _adaptiveCacheWeek = '';

async function loadAdaptiveWeights() {
    try {
        const db = require('./database');
        const data = await db.getAdaptiveWeights();
        if (data && data.weights && Object.keys(data.weights).length > 0) {
            _adaptiveCache     = data.weights;
            _adaptiveCacheWeek = data.week;
            console.log(`[AdaptiveWeights] Loaded ${Object.keys(data.weights).length} weights for ${data.week} (WR: ${data.winRate?.toFixed(1)}%)`);
        }
    } catch(e) {
        console.warn('[AdaptiveWeights] Could not load from DB:', e.message);
    }
}

// Get adaptive multiplier for an indicator (1.0 = neutral, >1 = boost, <1 = reduce)
function getAdaptiveMultiplier(indicatorName) {
    return _adaptiveCache[indicatorName] || 1.0;
}

// Auto-refresh every 6 hours
setInterval(loadAdaptiveWeights, 6 * 60 * 60 * 1000);
// Load on first use (after module system is ready)
setTimeout(loadAdaptiveWeights, 5000);

// ── Compute adaptive weights from recent trade history ────────────────────
// Call this weekly (from a cron or admin command) to update DB weights.
// trades = array of { reasons: String, result: 'WIN'|'LOSS' }
async function computeAndSaveAdaptiveWeights(trades) {
    if (!trades || trades.length < 10) return null;

    const indicatorWins   = {};  // { indicator: wins }
    const indicatorCounts = {};  // { indicator: total }
    ...
    for (const trade of trades) {
        ...
        for (const reason of reasons) {
            if (!reason) continue;
            const key = reason.split(' ')[0]; // first word = indicator name
            indicatorCounts[key] = (indicatorCounts[key] || 0) + 1;
            if (isWin) indicatorWins[key] = (indicatorWins[key] || 0) + 1;
        }
    }

    const overallWinRate = trades.filter(t => t.result === 'WIN').length / trades.length * 100;

    // Build weight multipliers: winRate / overallWinRate (clamped 0.5–2.0)
    const weights = {};
    for (const [key, count] of Object.entries(indicatorCounts)) {
        if (count < 3) continue; // need minimum sample
        const wr  = (indicatorWins[key] || 0) / count;
        const rel = overallWinRate > 0 ? (wr / (overallWinRate / 100)) : 1.0;
        weights[key] = Math.min(2.0, Math.max(0.5, parseFloat(rel.toFixed(3))));
    }

    try {
        const db = require('./database');
        const week = await db.saveAdaptiveWeights(weights, trades.length, overallWinRate);
        _adaptiveCache     = weights;
        _adaptiveCacheWeek = week;
        console.log(`[AdaptiveWeights] Saved ${Object.keys(weights).length} weights for ${week}`);
        return { week, weights, sampleSize: trades.length, winRate: overallWinRate };
    } catch(e) {
        console.warn('[AdaptiveWeights] Save failed:', e.message);
        return null;
    }
}
...
module.exports = { getDynamicWeights, w, loadAdaptiveWeights, getAdaptiveMultiplier, computeAndSaveAdaptiveWeights };
