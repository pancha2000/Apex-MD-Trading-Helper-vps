...
async function _run14FactorAnalysisImpl(coin, timeframe = '15m') {
    ...
    const [currentCandles, candles5m, candles1H, candles4H, candlesDaily, fundingRaw, btcCandles1H, btcCandles4H] = await Promise.all([
        ...
        coin !== 'BTCUSDT' ? binance.getKlineDataFromCache('BTCUSDT', '1h', 60).catch(() => null) : Promise.resolve(null),
        coin !== 'BTCUSDT' ? binance.getKlineDataFromCache('BTCUSDT', '4h', 60).catch(() => null) : Promise.resolve(null),
    ...
    // ── 4c. HTF OBs — declared here so confirmEntry5m can use ob4H ───
    ...
    // ── 4c. HTF OBs — already declared above before confirmEntry5m ──
