...
// ── confirmEntry5m v2 — 5m check + HTF gate ──────────────────────────
// Original: 5 checks (RSI, EMA21, candle, pattern, momentum)
// Upgrade:  + 4H trend alignment gate + daily bias gate + OB proximity
// HTF gates are HARD — if 4H is against direction, score capped at 2
function confirmEntry5m(candles5m, direction, candles4H = null, candlesDaily = null, ob4H = null) {
    ...
    // ── 5m micro checks (same as before) ─────────────────────────────
    if (isLong) {
    ...
    // ── HTF Gate 1: 4H trend alignment ───────────────────────────────
    let htfGatePass = true;
    let htfNote     = '';
    if (candles4H && candles4H.length >= 50) {
        const ema50_4H   = parseFloat(calculateEMA(candles4H, 50));
        const price4H    = parseFloat(candles4H[candles4H.length - 1][4]);
        const trend4H    = price4H > ema50_4H ? 'BULL' : 'BEAR';
        const htfAligned = (isLong && trend4H === 'BULL') || (!isLong && trend4H === 'BEAR');
        const htfAgainst = (isLong && trend4H === 'BEAR') || (!isLong && trend4H === 'BULL');

        if (htfAligned) {
            score++;
            reasons.push(`4H trend aligned ✅ (${trend4H})`);
            htfNote = `4H: ${trend4H} ✅`;
        } else if (htfAgainst) {
            // Hard cap — trading against 4H trend = max score 2 (no confirmation)
            score = Math.min(score, 2);
            warnings.push(`4H trend AGAINST (${trend4H}) ⛔ — score capped`);
            htfGatePass = false;
            htfNote     = `4H: ${trend4H} ⛔ AGAINST`;
        }
    }

    // ── HTF Gate 2: Daily trend alignment ────────────────────────────
    if (candlesDaily && candlesDaily.length >= 50) {
        const ema50_1D = parseFloat(calculateEMA(candlesDaily, 50));
        const price1D  = parseFloat(candlesDaily[candlesDaily.length - 1][4]);
        const trend1D  = price1D > ema50_1D ? 'BULL' : 'BEAR';
        const dayAlign = (isLong && trend1D === 'BULL') || (!isLong && trend1D === 'BEAR');
        const dayAgainst = !dayAlign;

        if (dayAlign) {
            score++;
            reasons.push(`Daily trend aligned ✅`);
        } else if (dayAgainst) {
            warnings.push(`Daily trend against ⚠️ — high risk`);
            score = Math.max(0, score - 1);
        }
    }

    // ── HTF Gate 3: 4H OB proximity ──────────────────────────────────
    if (ob4H) {
        const obForDir = isLong ? ob4H.bullish : ob4H.bearish;
        if (obForDir) {
            const obMid = (parseFloat(obForDir.bottom) + parseFloat(obForDir.top)) / 2;
            const dist  = Math.abs(lastClose - obMid) / lastClose * 100;
            if (dist < 1.5) {
                score++;
                reasons.push(`Near 4H OB zone (${dist.toFixed(1)}%) 🏦`);
            }
        }
    }

    const maxScore  = candles4H ? (candlesDaily ? 8 : 7) : 5;
    const threshold = candles4H ? 4 : 3; // higher bar when HTF data available
    const confirmed = score >= threshold && htfGatePass;

    const status = confirmed
        ? `✅ ENTRY CONFIRMED (${score}/${maxScore}) — HTF: ${htfNote||'N/A'}\n   ✔️ ${reasons.join(' | ')}`
        : `⚠️ ENTRY NOT CONFIRMED (${score}/${maxScore}) — ${htfNote||'5m weak'}\n   ❌ ${warnings.join(' | ')}`;

    return { confirmed, score, maxScore, threshold, reasons, warnings, status, htfGatePass };
}
...
// ============================================================
// ── detectMTFOrderBlocks v2 — 15m + 1H + 4H triple confluence ──────────
// Upgraded from DOUBLE (15m+1H) to TRIPLE (15m+1H+4H).
// TRIPLE confluence = 3 timeframes agree → strongest institutional zone.
// DOUBLE remains valid (any 2 TFs align).
// candles4H is optional — falls back to DOUBLE mode if not supplied.
function detectMTFOrderBlocks(candles15m, candles1H, candles4H = null) {
    const ob15 = detectMTFOBs(candles15m);
    const ob1H = detectMTFOBs(candles1H);
    const ob4H = candles4H ? detectMTFOBs(candles4H) : null;
    const currentPrice = parseFloat(candles15m[candles15m.length - 1][4]);

    // ── Helper: check if two zones overlap ──────────────────────────────
    function zonesOverlap(a, b) {
        const aLow = parseFloat(a.bottom), aHigh = parseFloat(a.top);
        const bLow = parseFloat(b.bottom), bHigh = parseFloat(b.top);
        const oLow = Math.max(aLow, bLow), oHigh = Math.min(aHigh, bHigh);
        return oLow < oHigh ? { low: oLow, high: oHigh } : null;
    }
    ...
    if (ob15.bullish && ob1H.bullish) {
        const ov = zonesOverlap(ob15.bullish, ob1H.bullish);
        if (ov) {
            let bottom = ov.low, top = ov.high, strength = 'DOUBLE', tfs = '15m+1H';

            // Try to extend to 4H for TRIPLE
            if (ob4H?.bullish) {
                const ov2 = zonesOverlap(
                    { bottom: ov.low.toFixed(4), top: ov.high.toFixed(4) },
                    ob4H.bullish
                );
                if (ov2) {
                    bottom   = ov2.low;
            ...
            confluenceZone = {
                type: 'BULLISH', bottom: bottom.toFixed(4), top: top.toFixed(4),
                strength, tfs,
                display: `${strength === 'TRIPLE' ? '🔥🔥' : '🔥'} MTF OB Conf (${tfs}): $${bottom.toFixed(4)} – $${top.toFixed(4)}`,
            };
        }
    }
    ...
    if (ob15.bearish && ob1H.bearish) {
        const ov = zonesOverlap(ob15.bearish, ob1H.bearish);
        if (ov) {
            let bottom = ov.low, top = ov.high, strength = 'DOUBLE', tfs = '15m+1H';
            ...
            confluenceZone = {
                type: 'BEARISH', bottom: bottom.toFixed(4), top: top.toFixed(4),
                strength, tfs,
                display: `${strength === 'TRIPLE' ? '🔥🔥' : '🔥'} MTF OB Conf (${tfs}): $${bottom.toFixed(4)} – $${top.toFixed(4)}`,
            };
        }
    ...
    return { ob15, ob1H, ob4H, confluenceZone };
}
...
// ══════════════════════════════════════════════════════════════════
//  🆕 v8 HIGH-ACCURACY — Advanced Pattern Detection Engine
//  New patterns: Cypher, ABCD, Head & Shoulders, Triangle,
//                Three Drives, Elliott Wave, Multi-TF RSI Divergence
// ══════════════════════════════════════════════════════════════════

// ── Multi-Timeframe RSI Divergence ──────────────────────────────
// Checks divergence on 15m AND 1H simultaneously for confirmation
// Returns: { bull, bear, hidden_bull, hidden_bear, mtfConfirmed, display }
function checkMTFRSIDivergence(candles15m, candles1H = null) {
    function _divCheck(candles) {
        if (!candles || candles.length < 35) return { bull: false, bear: false, hbull: false, hbear: false };
        const highs = candles.map(c => parseFloat(c[2]));
        const lows  = candles.map(c => parseFloat(c[3]));
        const n     = candles.length;
        ...
        for (let i = 3; i < n - 2; i++) {
            if (lows[i]  < lows[i-1]  && lows[i]  < lows[i-2]  && lows[i]  < lows[i+1]  && lows[i]  < lows[i+2])  swingLows.push(i);
            if (highs[i] > highs[i-1] && highs[i] > highs[i-2] && highs[i] > highs[i+1] && highs[i] > highs[i+2]) swingHighs.push(i);
        }
        ...
        if (swingLows.length >= 2) {
            ...
            if (sl2 - sl1 >= 5) {
                const r1 = calculateRSI(candles.slice(Math.max(0, sl1-14), sl1+1), 14);
                const r2 = calculateRSI(candles.slice(Math.max(0, sl2-14), sl2+1), 14);
                // Regular bullish: price LL + RSI HL
                if (lows[sl2] < lows[sl1] && r2 > r1 && r2 < 50)  bull  = true;
                // Hidden bullish: price HL + RSI LL
                if (lows[sl2] > lows[sl1] && r2 < r1 && r2 < 55)  hbull = true;
            }
        }
        if (swingHighs.length >= 2) {
            const sh1 = swingHighs[swingHighs.length - 2], sh2 = swingHighs[swingHighs.length - 1];
            if (sh2 - sh1 >= 5) {
                const r1 = calculateRSI(candles.slice(Math.max(0, sh1-14), sh1+1), 14);
                const r2 = calculateRSI(candles.slice(Math.max(0, sh2-14), sh2+1), 14);
                // Regular bearish: price HH + RSI LH
                if (highs[sh2] > highs[sh1] && r2 < r1 && r2 > 50) bear  = true;
                // Hidden bearish: price LH + RSI HH
                if (highs[sh2] < highs[sh1] && r2 > r1 && r2 > 45) hbear = true;
            }
        }
        return { bull, bear, hbull, hbear };
    }

    const d15m = _divCheck(candles15m);
    const d1H  = _divCheck(candles1H);

    // MTF confirmed = same signal on both timeframes
    const mtfBull      = d15m.bull  && d1H.bull;
    const mtfBear      = d15m.bear  && d1H.bear;
    const mtfHBull     = d15m.hbull && d1H.hbull;
    const mtfHBear     = d15m.hbear && d1H.hbear;
    const mtfConfirmed = mtfBull || mtfBear || mtfHBull || mtfHBear;

    let display = '⚪ No RSI Divergence';
    if      (mtfBull)  display = '🚀 MTF Bull Divergence (15m+1H confirmed) 💪';
    else if (mtfBear)  display = '🔴 MTF Bear Divergence (15m+1H confirmed) 💪';
    else if (mtfHBull) display = '🟢 MTF Hidden Bull Div (Trend continuation ↗️)';
    else if (mtfHBear) display = '🔴 MTF Hidden Bear Div (Trend continuation ↘️)';
    else if (d15m.bull)  display = '🟡 15m Bull Divergence (1H unconfirmed)';
    else if (d15m.bear)  display = '🟡 15m Bear Divergence (1H unconfirmed)';
    else if (d15m.hbull) display = '🟢 15m Hidden Bull Div';
    else if (d15m.hbear) display = '🔴 15m Hidden Bear Div';

    return {
        bull: d15m.bull, bear: d15m.bear, hbull: d15m.hbull, hbear: d15m.hbear,
        mtfBull, mtfBear, mtfHBull, mtfHBear, mtfConfirmed,
        display,
    };
}

// ── Cypher Pattern ───────────────────────────────────────────────
// X-A-B-C-D: B=0.382-0.618 of XA, C=1.272-1.414 of XA, D=0.786 of XC
function checkCypherPattern(candles) {
    if (candles.length < 40) return { bull: false, bear: false, display: 'None' };
    const highs = candles.map(c => parseFloat(c[2]));
    const lows  = candles.map(c => parseFloat(c[3]));
    const n     = candles.length;
    const curP  = parseFloat(candles[n-1][4]);

    const swHL = [], swLL = [];
    for (let i = 2; i < n - 2; i++) {
        if (highs[i] > highs[i-1] && highs[i] > highs[i+1]) swHL.push({ i, v: highs[i] });
        if (lows[i]  < lows[i-1]  && lows[i]  < lows[i+1])  swLL.push({ i, v: lows[i]  });
    }
    const tol = 0.04;

    // Bullish Cypher: X(high) A(low) B(high) C(low->high extension) D(retracement of XC)
    for (let xi = 0; xi < swHL.length - 1; xi++) {
        const X = swHL[xi].v, Xi = swHL[xi].i;
        for (const aP of swLL.filter(p => p.i > Xi)) {
            const A = aP.v, Ai = aP.i;
            const XA = X - A; if (XA <= 0) continue;
            for (const bP of swHL.filter(p => p.i > Ai)) {
                const B = bP.v, Bi = bP.i;
                const XB = (X - B) / XA;
                if (XB < 0.382 - tol || XB > 0.618 + tol) continue;
                for (const cP of swLL.filter(p => p.i > Bi)) {
                    const C = cP.v;
                    const XC = X - C; if (XC <= 0) continue;
                    const AC_ext = (X - C) / XA;
                    if (AC_ext < 1.13 - tol || AC_ext > 1.414 + tol) continue;
                    // D = 0.786 of XC from X
                    const D = X - XC * 0.786;
                    if (Math.abs(curP - D) / D < 0.015) {
                        return { bull: true, bear: false, display: '🟢 Bullish Cypher PRZ 🎯 (High Accuracy)' };
                    }
                }
            }
        }
    }
    // Bearish Cypher: inverse
    for (let xi = 0; xi < swLL.length - 1; xi++) {
        const X = swLL[xi].v, Xi = swLL[xi].i;
        for (const aP of swHL.filter(p => p.i > Xi)) {
            const A = aP.v, Ai = aP.i;
            const XA = A - X; if (XA <= 0) continue;
            for (const bP of swLL.filter(p => p.i > Ai)) {
                const B = bP.v;
                const XB = (B - X) / XA;
                if (XB < 0.382 - tol || XB > 0.618 + tol) continue;
                for (const cP of swHL.filter(p => p.i > bP.i)) {
                    const C = cP.v;
                    const XC = C - X; if (XC <= 0) continue;
                    const ext = XC / XA;
                    if (ext < 1.13 - tol || ext > 1.414 + tol) continue;
                    const D = X + XC * 0.786;
                    if (Math.abs(curP - D) / D < 0.015) {
                        return { bull: false, bear: true, display: '🔴 Bearish Cypher PRZ 🎯 (High Accuracy)' };
                    }
                }
            }
        }
    }
    return { bull: false, bear: false, display: 'None' };
}

// ── ABCD Pattern ─────────────────────────────────────────────────
// Simple 4-point: BC = 0.618-0.786 of AB, CD = 1.272-1.618 of BC
function checkABCDPattern(candles) {
    if (!candles || candles.length < 30) return { bull: false, bear: false, display: 'None' };
    const highs = candles.map(c => parseFloat(c[2]));
    const lows  = candles.map(c => parseFloat(c[3]));
    const n     = candles.length;
    const curP  = parseFloat(candles[n-1][4]);
    ...
    // Bullish ABCD: A(high) → B(low) → C(high) → D(low=entry zone)
    if (swHL.length >= 2 && swLL.length >= 2) {
        const Aobj = swHL[swHL.length - 2];
        if (Aobj) {
            const A = Aobj.v, Ai = Aobj.i;
            // Find first swing low AFTER A
            const Bobj = swLL.find(p => p.i > Ai);
            if (Bobj) {
                const Bv = Bobj.v, Bi = Bobj.i;
                const AB = A - Bv;
                if (AB > 0) {
                    // Find first swing high AFTER B
                    const Cobj = swHL.find(p => p.i > Bi);
                    if (Cobj) {
                        const C = Cobj.v, Ci = Cobj.i;
                        const BC = C - Bv;
                        const BCratio = BC / AB;
                        if (BCratio >= 0.618 - tol && BCratio <= 0.786 + tol) {
                            // D projected from C using CD = 1.272-1.618 of BC
                            const D_127 = C - BC * 1.272;
                            const D_161 = C - BC * 1.618;
                            const dLow  = Math.min(D_127, D_161);
                            const dHigh = Math.max(D_127, D_161);
                            if (curP >= dLow * 0.985 && curP <= dHigh * 1.015) {
                                return { bull: true, bear: false, display: '🟢 Bullish ABCD PRZ' };
                            }
                        }
                    }
                }
    ...
    // Bearish ABCD: A(low) → B(high) → C(low) → D(high=short zone)
    if (swLL.length >= 2 && swHL.length >= 2) {
        const Aobj = swLL[swLL.length - 2];
        if (Aobj) {
            const A = Aobj.v, Ai = Aobj.i;
            const Bobj = swHL.find(p => p.i > Ai);
            if (Bobj) {
                const Bv = Bobj.v, Bi = Bobj.i;
                const AB = Bv - A;
                if (AB > 0) {
                    const Cobj = swLL.find(p => p.i > Bi);
                    if (Cobj) {
                        const C = Cobj.v, Ci = Cobj.i;
                        const BC = Bv - C;
                        const BCratio = BC / AB;
                        if (BCratio >= 0.618 - tol && BCratio <= 0.786 + tol) {
                            ...
                            const dHigh = Math.max(D_127, D_161);
                            if (curP >= dLow * 0.985 && curP <= dHigh * 1.015) {
                                return { bull: false, bear: true, display: '🔴 Bearish ABCD PRZ' };
                            }
                        }
                    }
                }
...
// Finds H&S top (bearish) and Inverse H&S (bullish)
function checkHeadShoulders(candles) {
    if (candles.length < 50) return { bull: false, bear: false, neckline: null, display: 'None' };
    const highs  = candles.map(c => parseFloat(c[2]));
    const lows   = candles.map(c => parseFloat(c[3]));
    const closes = candles.map(c => parseFloat(c[4]));
    const n      = candles.length;
    ...
    const swHL = [], swLL = [];
    for (let i = 3; i < n - 3; i++) {
        if (highs[i] > highs[i-1] && highs[i] > highs[i-2] && highs[i] > highs[i+1] && highs[i] > highs[i+2]) swHL.push({ i, v: highs[i] });
        if (lows[i]  < lows[i-1]  && lows[i]  < lows[i-2]  && lows[i]  < lows[i+1]  && lows[i]  < lows[i+2])  swLL.push({ i, v: lows[i] });
    }

    // H&S TOP: LS < Head > RS (all highs), LS ≈ RS (within 3%)
    if (swHL.length >= 3) {
        for (let hi = 1; hi < swHL.length - 1; hi++) {
            const LS  = swHL[hi-1].v, Head = swHL[hi].v, RS = swHL[hi+1].v;
            if (Head <= LS || Head <= RS) continue;
            const symm = Math.abs(LS - RS) / Head;
            if (symm > tol) continue;
            // Neckline = average of the two troughs between shoulders
            const trough1 = Math.min(...lows.slice(swHL[hi-1].i, swHL[hi].i));
            const trough2 = Math.min(...lows.slice(swHL[hi].i, swHL[hi+1].i));
            const neckline = (trough1 + trough2) / 2;
            if (curP < neckline * (1 + tol) && curP > neckline * 0.97) {
                return { bull: false, bear: true, neckline: neckline.toFixed(4), display: `🔴 Head & Shoulders TOP — Neckline $${neckline.toFixed(4)} ⚠️` };
            }
        }
    }

    // INVERSE H&S: LS > Head < RS (all lows), LS ≈ RS
    if (swLL.length >= 3) {
        for (let hi = 1; hi < swLL.length - 1; hi++) {
            const LS  = swLL[hi-1].v, Head = swLL[hi].v, RS = swLL[hi+1].v;
            if (Head >= LS || Head >= RS) continue;
            const symm = Math.abs(LS - RS) / Math.abs(Head);
            if (symm > tol) continue;
            const peak1 = Math.max(...highs.slice(swLL[hi-1].i, swLL[hi].i));
            const peak2 = Math.max(...highs.slice(swLL[hi].i, swLL[hi+1].i));
            const neckline = (peak1 + peak2) / 2;
            if (curP > neckline * (1 - tol) && curP < neckline * 1.03) {
                return { bull: true, bear: false, neckline: neckline.toFixed(4), display: `🟢 Inverse H&S — Neckline $${neckline.toFixed(4)} 🚀` };
            }
        }
    }
    return { bull: false, bear: false, neckline: null, display: 'None' };
}

// ── Triangle Pattern Detection ────────────────────────────────────
// Ascending (bull), Descending (bear), Symmetrical (breakout pending)
function checkTrianglePattern(candles) {
    if (candles.length < 30) return { type: 'NONE', bull: false, bear: false, apex: null, display: 'None' };
    const highs  = candles.map(c => parseFloat(c[2]));
    const lows   = candles.map(c => parseFloat(c[3]));
    const n      = candles.length;

    // Use last 20 candles for triangle analysis
    const slice = 20;
    const recentHighs = highs.slice(-slice);
    const recentLows  = lows.slice(-slice);

    // Least-squares slope of highs and lows
    function slope(arr) {
        const m = arr.length;
        let sumX = 0, sumY = 0, sumXY = 0, sumX2 = 0;
        arr.forEach((v, i) => { sumX += i; sumY += v; sumXY += i*v; sumX2 += i*i; });
        return (m * sumXY - sumX * sumY) / (m * sumX2 - sumX * sumX);
    }

    const sH = slope(recentHighs); // slope of highs
    const sL = slope(recentLows);  // slope of lows
    const tol = 0.0001;

    let type = 'NONE', bull = false, bear = false;
    let display = 'None';

    if (Math.abs(sH) < tol && sL > tol) {
        // Flat top + rising bottom = Ascending Triangle (bullish breakout)
        type = 'ASCENDING'; bull = true;
        display = '📐 Ascending Triangle — Bullish Breakout Pending 🚀';
    } else if (sH < -tol && Math.abs(sL) < tol) {
        // Falling top + flat bottom = Descending Triangle (bearish breakout)
        type = 'DESCENDING'; bear = true;
        display = '📐 Descending Triangle — Bearish Breakout Pending 📉';
    } else if (sH < -tol && sL > tol) {
        // Both converging = Symmetrical Triangle (neutral, await breakout)
        type = 'SYMMETRICAL';
        display = '📐 Symmetrical Triangle ⚡ (Breakout Imminent)';
    }

    // Estimate apex (bars until convergence)
    let apex = null;
    if (type !== 'NONE' && sH !== sL) {
        const curH = recentHighs[slice-1], curL = recentLows[slice-1];
        const barsToApex = Math.abs((curH - curL) / (sL - sH));
        apex = Math.round(barsToApex);
        if (apex > 0 && apex < 50) display += ` | ~${apex} candles to apex`;
    }

    return { type, bull, bear, apex, display };
}

// ── Three Drives Pattern ─────────────────────────────────────────
// 3 symmetric drives to new highs/lows, with Fib-based retracements
function checkThreeDrives(candles) {
    if (candles.length < 40) return { bull: false, bear: false, display: 'None' };
    const highs  = candles.map(c => parseFloat(c[2]));
    const lows   = candles.map(c => parseFloat(c[3]));
    const n      = candles.length;
    ...
    // Bearish Three Drives: 3 ascending highs, each ~1.272 of prior drive
    if (swHL.length >= 3) {
        const [D1, D2, D3] = swHL.slice(-3);
        const drive1 = D2.v - D1.v, drive2 = D3.v - D2.v;
        if (drive1 > 0 && drive2 > 0) {
            const ratio = drive2 / drive1;
            if (Math.abs(ratio - 1.272) < tol || Math.abs(ratio - 1.618) < tol) {
                if (Math.abs(curP - D3.v) / D3.v < 0.015) {
                    return { bull: false, bear: true, display: '🔴 Three Drives TOP — Reversal Zone ⚠️' };
                }
            }
        }
    }
    // Bullish Three Drives: 3 descending lows
    if (swLL.length >= 3) {
        const [D1, D2, D3] = swLL.slice(-3);
        const drive1 = D1.v - D2.v, drive2 = D2.v - D3.v;
        if (drive1 > 0 && drive2 > 0) {
            const ratio = drive2 / drive1;
            if (Math.abs(ratio - 1.272) < tol || Math.abs(ratio - 1.618) < tol) {
                if (Math.abs(curP - D3.v) / D3.v < 0.015) {
                    return { bull: true, bear: false, display: '🟢 Three Drives BOTTOM — Reversal Zone 🚀' };
                }
            }
        }
    }
...
// ── Elliott Wave Basic Detection ─────────────────────────────────
// Detects likely Wave 3 (strongest, longest) and Wave 5 completion
// Returns: { wave, phase, isBull, isBear, display }
function detectElliottWave(candles) {
    if (candles.length < 50) return { wave: 0, phase: 'UNKNOWN', isBull: false, isBear: false, display: 'None' };
    const closes = candles.map(c => parseFloat(c[4]));
    const highs  = candles.map(c => parseFloat(c[2]));
    const lows   = candles.map(c => parseFloat(c[3]));
    const n      = closes.length;
    ...
    for (let i = 3; i < n - 3; i++) {
        ...
        if (isHigh) pivots.push({ i, v: highs[i], type: 'H' });
        if (isLow)  pivots.push({ i, v: lows[i],  type: 'L' });
    }
    pivots.sort((a, b) => a.i - b.i);

    if (pivots.length < 5) return { wave: 0, phase: 'INSUFFICIENT', isBull: false, isBear: false, display: 'None' };

    // Look for 5-wave impulse structure in last 8 pivots
    const recent = pivots.slice(-8);

    // Try to match L-H-L-H-L (bullish 5-wave: waves 1,2,3,4,5)
    for (let i = 0; i < recent.length - 4; i++) {
        const [w0, w1, w2, w3, w4] = recent.slice(i, i+5);
        if (!w0 || !w1 || !w2 || !w3 || !w4) continue;

        // Bullish impulse: L-H-L-H-L (each H higher, each L higher)
        if (w0.type==='L' && w1.type==='H' && w2.type==='L' && w3.type==='H' && w4.type==='L') {
            const wave1 = w1.v - w0.v;
            const wave3 = w3.v - w2.v;
            const wave5end = w4.v;
            const rule1 = w2.v > w0.v;           // Wave 2 doesn't go below 0
            const rule2 = wave3 > wave1 * 0.9;    // Wave 3 not shortest
            const rule3 = w3.v > w1.v;            // Wave 3 highest
            if (rule1 && rule2 && rule3) {
                // Are we near wave 5 bottom? (potential reversal up)
                const distFromW5 = Math.abs(curP - wave5end) / wave5end;
                if (distFromW5 < 0.02) {
                    return { wave: 5, phase: 'WAVE_5_END', isBull: true, isBear: false,
                        display: `🌊 Elliott Wave 5 Bottom — High Probability Reversal UP 🚀` };
                }
                // Near wave 3 peak? (warning — could correct)
                if (Math.abs(curP - w3.v) / w3.v < 0.02) {
                    return { wave: 3, phase: 'WAVE_3_TOP', isBull: false, isBear: true,
                        display: `🌊 Elliott Wave 3 TOP — Expect Wave 4 Correction ⚠️` };
                }
            }
        }

        // Bearish impulse: H-L-H-L-H
        if (w0.type==='H' && w1.type==='L' && w2.type==='H' && w3.type==='L' && w4.type==='H') {
            const wave1 = w0.v - w1.v;
            const wave3 = w2.v - w3.v;
            const rule1 = w2.v < w0.v;
            const rule2 = wave3 > wave1 * 0.9;
            const rule3 = w3.v < w1.v;
            if (rule1 && rule2 && rule3) {
                const distFromW5 = Math.abs(curP - w4.v) / w4.v;
                if (distFromW5 < 0.02) {
                    return { wave: 5, phase: 'WAVE_5_TOP', isBull: false, isBear: true,
                        display: `🌊 Elliott Wave 5 TOP — High Probability Reversal DOWN 📉` };
                }
            }
        }
    }

    // Simplified: Check if in likely Wave 3 (strongest momentum phase)
    // Wave 3: price broke above wave 1 high with expanding volume/momentum
    const recentH = recent.filter(p => p.type === 'H');
    const recentL = recent.filter(p => p.type === 'L');
    if (recentH.length >= 2 && recentL.length >= 2) {
        const lastH = recentH[recentH.length-1].v;
        const prevH = recentH[recentH.length-2].v;
        const lastL = recentL[recentL.length-1].v;
        const prevL = recentL[recentL.length-2].v;

        if (lastH > prevH && lastL > prevL && curP > lastH * 0.98) {
            return { wave: 3, phase: 'POSSIBLE_WAVE_3', isBull: true, isBear: false,
                display: `🌊 Possible Elliott Wave 3 — Strongest impulse phase 🚀` };
        }
        if (lastH < prevH && lastL < prevL && curP < lastL * 1.02) {
            return { wave: 3, phase: 'POSSIBLE_WAVE_3_BEAR', isBull: false, isBear: true,
                display: `🌊 Possible Elliott Wave 3 DOWN — Strong bearish impulse 📉` };
        }
    }

    return { wave: 0, phase: 'UNCLEAR', isBull: false, isBear: false, display: 'None' };
}

// ── Fibonacci Time Zone ───────────────────────────────────────────
// Time-based fib zones: 1, 2, 3, 5, 8, 13, 21 bars from last swing
// Returns if current bar is AT a significant fib time zone
function checkFibTimeZone(candles) {
    if (candles.length < 30) return { atZone: false, zoneNum: 0, display: 'None' };
    const n      = candles.length;
    const highs  = candles.map(c => parseFloat(c[2]));
    const lows   = candles.map(c => parseFloat(c[3]));

    // Find last significant swing pivot
    let lastSwingIdx = -1;
    for (let i = n - 3; i >= n - 30; i--) {
    ...
    if (lastSwingIdx < 0) return { atZone: false, zoneNum: 0, display: 'None' };

    const barsFromSwing = n - 1 - lastSwingIdx;
    const fibNums = [1, 2, 3, 5, 8, 13, 21, 34];

    for (const fib of fibNums) {
        if (Math.abs(barsFromSwing - fib) <= 1) {
            return { atZone: true, zoneNum: fib, display: `⏱️ Fib Time Zone ${fib} — ${barsFromSwing} bars from swing` };
        }
    }
    return { atZone: false, zoneNum: 0, display: 'None' };
}
...
module.exports = {
    ...
    calculateCCI,
    detectMomentumShift,
    // 🆕 v7 PRO VVIP — advanced exports
    checkHiddenDivergence,
    getMTFFibEntrySystem,
    getEntryQualityGate,
    getPostTP1SL,
    getBTCContext,
    // 🆕 v8 HIGH-ACCURACY — new patterns & MTF divergence
    checkMTFRSIDivergence,
    checkCypherPattern,
    checkABCDPattern,
    checkHeadShoulders,
    checkTrianglePattern,
    checkThreeDrives,
    checkFibTimeZone,
};
...
// ═══════════════════════════════════════════════════════════════
// 🆕 ENHANCED ENTRY INTELLIGENCE SYSTEM
// ═══════════════════════════════════════════════════════════════

// ─────────────────────────────────────────────────────────────
// Hidden Divergence — trend continuation signal
// Regular Div = reversal | Hidden Div = continuation
// Hidden Bull: price higher low + RSI lower low (uptrend continues)
// Hidden Bear: price lower high + RSI higher high (downtrend continues)
// ─────────────────────────────────────────────────────────────
function checkHiddenDivergence(candles) {
    if (candles.length < 35) return { bull: false, bear: false, display: 'None' };
    const closes = candles.map(c => parseFloat(c[4]));
    const lows   = candles.map(c => parseFloat(c[3]));
    const highs  = candles.map(c => parseFloat(c[2]));
    const n      = candles.length;
    ...
    let hiddenBull = false, hiddenBear = false;

    // Hidden Bullish: price HL (higher low) + RSI LL (lower low) → uptrend continues
    if (swingLows.length >= 2) {
        const sl1 = swingLows[swingLows.length - 2];
        const sl2 = swingLows[swingLows.length - 1];
        if (sl2 - sl1 >= 5) {
            const rsi1 = calculateRSI(candles.slice(Math.max(0, sl1-14), sl1+1), 14);
            const rsi2 = calculateRSI(candles.slice(Math.max(0, sl2-14), sl2+1), 14);
            // Price made HIGHER low, RSI made LOWER low → hidden bull div
            if (lows[sl2] > lows[sl1] && rsi2 < rsi1 && rsi2 < 55) hiddenBull = true;
        }
    }

    // Hidden Bearish: price LH (lower high) + RSI HH (higher high) → downtrend continues
    if (swingHighs.length >= 2) {
        const sh1 = swingHighs[swingHighs.length - 2];
        const sh2 = swingHighs[swingHighs.length - 1];
        if (sh2 - sh1 >= 5) {
            const rsi1 = calculateRSI(candles.slice(Math.max(0, sh1-14), sh1+1), 14);
            const rsi2 = calculateRSI(candles.slice(Math.max(0, sh2-14), sh2+1), 14);
            if (highs[sh2] < highs[sh1] && rsi2 > rsi1 && rsi2 > 45) hiddenBear = true;
        }
    }

    const display = hiddenBull ? '🟢 Hidden Bull Divergence (Trend Continuation UP ↗️)'
                  : hiddenBear ? '🔴 Hidden Bear Divergence (Trend Continuation DOWN ↘️)'
                  : 'None';

    return { bull: hiddenBull, bear: hiddenBear, display };
}

// ─────────────────────────────────────────────────────────────
// Multi-Timeframe Fibonacci Smart Entry System
// Uses 4H swing for entry zones, 1H for TP refinement
// Returns: entryZone, tpLevels, slLevel, quality grade
// ─────────────────────────────────────────────────────────────
function getMTFFibEntrySystem(candles15m, candles1H, candles4H, direction) {
    const isLong = direction === 'LONG';

    // ── 4H Fibonacci: Main entry zones ──
    const fib4H = getFibonacciLevels(candles4H || candles15m, direction);

    // ── 1H Fibonacci: TP refinement ──
    const fib1H = getFibonacciLevels(candles1H || candles15m, direction);

    // ── 15m Fibonacci: Fine-tune entry ──
    const fib15m = getFibonacciLevels(candles15m, direction);

    const currentPrice = parseFloat(candles15m[candles15m.length-1][4]);

    // Entry zones (OTE = 61.8-78.6% retracement from swing)
    const zones = [];

    if (isLong) {
        // 4H OTE zones (strongest — institutional)
        if (fib4H.key618) zones.push({ price: parseFloat(fib4H.key618), label: '4H Fib 61.8%', strength: 'STRONG', timeframe: '4H' });
        if (fib4H.key786) zones.push({ price: parseFloat(fib4H.key786), label: '4H Fib 78.6%', strength: 'STRONG', timeframe: '4H' });
        if (fib4H.key50)  zones.push({ price: parseFloat(fib4H.key50),  label: '4H Fib 50%',   strength: 'MEDIUM', timeframe: '4H' });
        // 1H zones (secondary)
        if (fib1H.key618) zones.push({ price: parseFloat(fib1H.key618), label: '1H Fib 61.8%', strength: 'MEDIUM', timeframe: '1H' });
        if (fib1H.key786) zones.push({ price: parseFloat(fib1H.key786), label: '1H Fib 78.6%', strength: 'MEDIUM', timeframe: '1H' });
    } else {
        // SHORT: retracement zones above current price
        const s4H = parseFloat(fib4H.swingLow), h4H = parseFloat(fib4H.swingHigh);
        const range4H = h4H - s4H;
        if (range4H > 0) {
            zones.push({ price: s4H + range4H * 0.618, label: '4H Fib 61.8% SHORT', strength: 'STRONG', timeframe: '4H' });
            zones.push({ price: s4H + range4H * 0.786, label: '4H Fib 78.6% SHORT', strength: 'STRONG', timeframe: '4H' });
        }
        const s1H = parseFloat(fib1H.swingLow), h1H = parseFloat(fib1H.swingHigh);
        const range1H = h1H - s1H;
        if (range1H > 0) {
            zones.push({ price: s1H + range1H * 0.618, label: '1H Fib 61.8% SHORT', strength: 'MEDIUM', timeframe: '1H' });
        }
    }
    ...
    // TP levels using 4H extensions
    const tps = [];
    if (isLong) {
        if (fib4H.ext1272) tps.push({ price: parseFloat(fib4H.ext1272), label: '4H Fib 1.272 TP1' });
        if (fib4H.ext1618) tps.push({ price: parseFloat(fib4H.ext1618), label: '4H Fib 1.618 TP2 🎯' });
        if (fib4H.ext2618) tps.push({ price: parseFloat(fib4H.ext2618), label: '4H Fib 2.618 TP3 🚀' });
        // Also add 1H extensions as intermediate TPs
        if (fib1H.ext1618 && parseFloat(fib1H.ext1618) < (tps[0]?.price || Infinity)) {
            tps.unshift({ price: parseFloat(fib1H.ext1618), label: '1H Fib 1.618 TP1' });
        }
    } else {
        const s4H = parseFloat(fib4H.swingLow), h4H = parseFloat(fib4H.swingHigh);
        const range4H = h4H - s4H;
        if (range4H > 0) {
            tps.push({ price: h4H - range4H * 1.272, label: '4H Fib 1.272 TP1' });
            tps.push({ price: h4H - range4H * 1.618, label: '4H Fib 1.618 TP2 🎯' });
            tps.push({ price: h4H - range4H * 2.618, label: '4H Fib 2.618 TP3 🚀' });
        }
    }

    // Check if current price is inside a zone (within 1%)
    const nearestZone = zones[0];
    const distPct = nearestZone ? Math.abs(nearestZone.price - currentPrice) / currentPrice * 100 : 999;
    const atZone = distPct < 1.0;

    // Zone bypass check — REAL bypass only when price ran 70%+ of the way to TP1.
    // "Price just passed the entry zone going up" is NOT a bypass — it's a LONG in motion.
    // Only mark bypassed when:
    //   LONG: price is already 70%+ of (TP1 - zone) distance from zone
    //   SHORT: price is already 70%+ of (zone - TP1) distance below zone
    // In that case, the trade is nearly complete and a new entry here is late / bad RRR.
    let zoneBypassed = false;
    if (zones.length > 0 && tps.length > 0) {
        const z0  = zones[0].price;
        const tp0 = tps[0].price;
        if (isLong && tp0 > z0) {
            const totalMove = tp0 - z0;
            const priceMove = currentPrice - z0;
            zoneBypassed = priceMove > totalMove * 0.70; // price 70%+ to TP1
        } else if (!isLong && tp0 < z0) {
            const totalMove = z0 - tp0;
            const priceMove = z0 - currentPrice;
            zoneBypassed = priceMove > totalMove * 0.70;
        }
    }

    return {
        zones: zones.slice(0, 3),
        tps:   tps.slice(0, 3).filter(t => t.price > 0),
        nearestZone,
...
// ─────────────────────────────────────────────────────────────
// Entry Quality Gate — Should we enter NOW or WAIT?
// Returns: ENTER_NOW | WAIT_FOR_ZONE | INVALIDATED | STRONG_ENTRY
// ─────────────────────────────────────────────────────────────
function getEntryQualityGate(currentPrice, mtfFib, ob, direction, confirmation, score) {
    const isLong   = direction === 'LONG';
    const nearZone = mtfFib?.nearestZone;
    const distPct  = mtfFib?.distPct ?? 999;

    // ── REAL bypass: price is 70%+ of the way to TP1 ─────────────────────────
    // At this point RRR is too poor for a new entry. Wait for structure reset.
    if (mtfFib?.zoneBypassed) {
        return {
            grade:      'INVALIDATED',
            action:     '⏳ WAIT — TP1 Zone Reached',
            detail:     'Price is 70%+ of the way to TP1. RRR too low for new entry here. Wait for pullback to Fib zone.',
            color:      'red',
            blockEntry: true,
        };
    }

    // ── Price between entry zone and TP (momentum trade in progress) ──────────
    // This is NOT a bypass — it is a valid continuation entry at current price.
    // Happens when: entry zone was below, price moved past it toward TP (< 70%).
    const pricePassedZone = nearZone && (
        (isLong  && currentPrice > nearZone.price * 1.008) ||
        (!isLong && currentPrice < nearZone.price * 0.992)
    );
    if (pricePassedZone && !mtfFib?.zoneBypassed) {
        const obConfirmedC = confirmation?.confirmed === true;
        return {
            grade:      obConfirmedC ? 'B+ CONTINUATION' : 'B CONTINUATION',
            action:     obConfirmedC ? '✅ ENTER NOW — Momentum Continuation' : '⚠️ ENTER — Momentum (no OB confirm)',
            detail:     `Price moved past ${nearZone.label}. Momentum trade — use current price as entry. TP/SL intact.`,
            color:      obConfirmedC ? 'green' : 'yellow',
            blockEntry: false,
        };
    }

    // OB confirmed + at Fib zone = strongest possible entry
    const obConfirmed   = confirmation?.confirmed === true;
    const atFibOTE      = distPct < 0.8 && nearZone?.strength === 'STRONG';
    const nearFibZone   = distPct < 2.0;
    const farFromZone   = distPct > 3.0;

    if (obConfirmed && atFibOTE) {
        return {
            grade:    'A+ SNIPER',
            action:   '✅ ENTER NOW — SNIPER ENTRY',
            detail:   `OB Confirmed + ${nearZone.label} Confluence. Maximum probability setup.`,
            color:    'green',
            blockEntry: false,
        };
    }
    if (obConfirmed && nearFibZone) {
        return {
            grade:    'A STRONG',
            action:   '✅ ENTER — Strong Confluence',
            detail:   `OB Confirmed. Near ${nearZone?.label||'Fib'} (${distPct}% away).`,
            color:    'green',
            blockEntry: false,
        };
    }
    if (obConfirmed && farFromZone) {
        return {
            grade:    'B+ MARKET',
            action:   '⚠️ MARKET ENTRY — OB Confirmed but zone far',
            detail:   `OB confirmed. Nearest zone ${nearZone?.label||'Fib'} is ${distPct}% away. RR reduced.`,
            color:    'yellow',
            blockEntry: false,
        };
    }
    if (!obConfirmed && atFibOTE) {
        return {
            grade:    'B WAIT_CONFIRM',
            action:   '⏳ WAIT — At Fib zone, awaiting OB confirmation',
            detail:   `Price at ${nearZone.label}. Wait for bullish close to confirm entry.`,
            color:    'yellow',
            blockEntry: true,
        };
    }
    if (farFromZone) {
        return {
            grade:    'C WAIT_ZONE',
            action:   '⏳ WAIT FOR ZONE',
            detail:   `Nearest entry zone (${nearZone?.label||'Fib'}) is ${distPct}% away. Set limit order at $${nearZone?.price?.toFixed(4)||'—'}.`,
            color:    'orange',
            blockEntry: true,
        };
    }
    ...
    return {
        grade:    'B',
        action:   '📊 STANDARD — Score-based entry',
        detail:   `No strong zone confluence. Trade based on ${score}/100 score.`,
        color:    'yellow',
        blockEntry: false,
    };
}
...
// ─────────────────────────────────────────────────────────────
// Smart TP/SL after TP1 hit — SL → entry + buffer (not just breakeven)
// TP1 hit = 33% profit taken → SL should lock in at least fees + small profit
// ─────────────────────────────────────────────────────────────
function getPostTP1SL(entryPrice, direction, tp1Price, feePct = 0.0004) {
    const entry  = parseFloat(entryPrice);
    const tp1    = parseFloat(tp1Price);
    const isLong = direction === 'LONG';

    // Lock in: entry + (33% of TP1 gain × 0.5) + fee buffer
    // Rationale: 33% profit already booked, so SL should protect remaining 67%
    // from going negative after fees
    const tp1Gain   = Math.abs(tp1 - entry);
    const feeBuffer = entry * feePct * 2; // round trip fee
    const lockIn    = tp1Gain * 0.15;     // lock in 15% of TP1 move on remaining position

    const newSL = isLong
        ? entry + feeBuffer + lockIn   // slightly above entry
        : entry - feeBuffer - lockIn;  // slightly below entry

    const profitLocked = Math.abs(newSL - entry) / entry * 100;

    return {
        sl:            parseFloat(newSL.toFixed(4)),
        label:         `Post-TP1 Lock (+${profitLocked.toFixed(2)}%)`,
        profitLocked:  parseFloat(profitLocked.toFixed(3)),
        isProfit:      true, // This SL hit = small profit, never a loss
    };
}

// ─────────────────────────────────────────────────────────────
// BTC Real-Time Context for altcoin analysis
// ─────────────────────────────────────────────────────────────
async function getBTCContext(candles1H_btc, candles4H_btc) {
    if (!candles1H_btc || !candles4H_btc) return { trend: 'UNKNOWN', score: 0, display: '⚪ BTC N/A' };
    try {
        const btcClose1H = parseFloat(candles1H_btc[candles1H_btc.length-1][4]);
        const btcClose4H = parseFloat(candles4H_btc[candles4H_btc.length-1][4]);
        const btcEma50_1H = parseFloat(calculateEMA(candles1H_btc.slice(-60), 50));
        const btcEma50_4H = parseFloat(calculateEMA(candles4H_btc.slice(-60), 50));
        const btcRsi1H   = calculateRSI(candles1H_btc.slice(-20), 14);

        const bull1H = btcClose1H > btcEma50_1H;
        const bull4H = btcClose4H > btcEma50_4H;
        const rsiOk  = btcRsi1H > 45 && btcRsi1H < 75;

        const trend  = bull4H && bull1H ? 'BULL' : !bull4H && !bull1H ? 'BEAR' : 'NEUTRAL';
        const btcScore = (bull4H ? 2 : -2) + (bull1H ? 1 : -1) + (rsiOk ? 1 : 0);

        return {
            trend,
            score:   btcScore,
            rsi1H:   parseFloat(btcRsi1H.toFixed(1)),
            bull4H, bull1H,
            display: trend === 'BULL' ? `₿ BTC Bullish (4H+1H) RSI:${btcRsi1H.toFixed(0)} 🟢`
                   : trend === 'BEAR' ? `₿ BTC Bearish (4H+1H) RSI:${btcRsi1H.toFixed(0)} 🔴`
                   : `₿ BTC Mixed — Wait for clarity ⚠️`,
        };
    } catch(e) {
        return { trend: 'UNKNOWN', score: 0, display: '⚪ BTC Context Error' };
    }
}
