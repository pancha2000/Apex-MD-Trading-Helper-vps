...
const SaasUserSchema = new mongoose.Schema({
    ...
    minScoreThreshold: { type: Number, default: 20, min: 10, max: 90 }, // per-user signal quality threshold
});
...
async function setUserMinScore(userId, score) {
    await connect();
    const s = Math.max(10, Math.min(90, parseInt(score) || 20));
    await SaasUser.findByIdAndUpdate(userId, { minScoreThreshold: s });
}
