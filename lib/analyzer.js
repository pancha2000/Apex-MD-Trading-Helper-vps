...
async function _run14FactorAnalysisImpl(coin, timeframe = '15m') {
    ...
    const ema50_15m = candles?.length >= 50 ? parseFloat(indicators.calculateEMA(currentCandles, 50)) : ema200;
    ...
    if (!dailyAligned && dailyTrend !== 'Unknown ⚪') {
        if (direction === 'LONG')  { longScore  = Math.max(0, longScore  - 10); longR.push('⚠️ Against Daily Trend'); }
        if (direction === 'SHORT') { shortScore = Math.max(0, shortScore - 10); shortR.push('⚠️ Against Daily Trend'); }
    } else if (dailyAligned) {
        if (direction === 'LONG')  { longScore  += 5; longR.push('✅ Daily Aligned'); }
        if (direction === 'SHORT') { shortScore += 5; shortR.push('✅ Daily Aligned'); }
    }
    ...
    if (!(trend1H.includes('Bullish') && trend4H.includes('Bullish')) && 
        ...
        // HTF conflict — reduce both scores
