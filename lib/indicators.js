...
// ═══════════════════════════════════════════════════════════════════
// 🆕 NEW INDICATORS — Squeeze Momentum, Waddah Attar, Hull Suite,
//    KAMA, VPCI, Stiffness, Hurst Exponent
// ═══════════════════════════════════════════════════════════════════

// ─────────────────────────────────────────────────────────────────
// Squeeze Momentum (TTM Squeeze)
// Detects when market is coiling (BB inside KC = squeeze)
// Then momentum bar direction = breakout direction
// ─────────────────────────────────────────────────────────────────
function calculateSqueezeMomentum(candles, bbPeriod = 20, kcPeriod = 20, kcMult = 1.5) {
    if (candles.length < bbPeriod + 5) return { squeeze: false, firing: false, bullish: false, bearish: false, value: 0, display: '⚪ Squeeze N/A' };
    const closes = candles.map(c => parseFloat(c[4]));
    const highs  = candles.map(c => parseFloat(c[2]));
    const lows   = candles.map(c => parseFloat(c[3]));
    const n = candles.length;
    ...
    const std = Math.sqrt(closes.slice(-bbPeriod).reduce((s,v)=>s+Math.pow(v-sma,2),0) / bbPeriod);
    const bbUpper = sma + 2 * std, bbLower = sma - 2 * std;

    // Keltner Channel
    const atr = parseFloat(calculateATR(candles.slice(-kcPeriod), kcPeriod));
    const kcUpper = sma + kcMult * atr, kcLower = sma - kcMult * atr;

    // Squeeze: BB inside KC
    const inSqueeze = bbUpper < kcUpper && bbLower > kcLower;

    // Momentum value (delta from midpoint of range)
    const highest = Math.max(...highs.slice(-bbPeriod));
    const lowest  = Math.min(...lows.slice(-bbPeriod));
    const midHL   = (highest + lowest) / 2;
    const midSMA  = (sma + midHL) / 2;
    const momentum = closes[n-1] - midSMA;

    // Prev momentum for direction
    const prevHighest = Math.max(...highs.slice(-bbPeriod-1, -1));
    const prevLowest  = Math.min(...lows.slice(-bbPeriod-1, -1));
    const prevMidHL   = (prevHighest + prevLowest) / 2;
    const prevSMA     = closes.slice(-bbPeriod-1,-1).reduce((s,v)=>s+v,0) / bbPeriod;
    const prevMidSMA  = (prevSMA + prevMidHL) / 2;
    const prevMomentum = closes[n-2] - prevMidSMA;

    const bullish = momentum > 0 && momentum > prevMomentum;
    const bearish = momentum < 0 && momentum < prevMomentum;
    const firing  = !inSqueeze && (Math.abs(prevMomentum) < Math.abs(momentum));

    let display = inSqueeze ? '🔴 SQUEEZE ON — Coiling, breakout imminent!'
                : firing && bullish ? '💥 SQUEEZE FIRED BULL 🚀 — Momentum exploding UP'
                : firing && bearish ? '💥 SQUEEZE FIRED BEAR 📉 — Momentum exploding DOWN'
                : bullish ? '🟢 Squeeze Momentum Bullish' : bearish ? '🔴 Squeeze Momentum Bearish' : '⚪ Squeeze Neutral';

    return { squeeze: inSqueeze, firing, bullish, bearish, value: parseFloat(momentum.toFixed(4)), display };
}

// ─────────────────────────────────────────────────────────────────
// Waddah Attar Explosion
// Trend strength: MACD delta × BB width. When explosion bar > deadzone = signal
// ─────────────────────────────────────────────────────────────────
function calculateWaddahAttar(candles, fastLength = 20, slowLength = 40, bbPeriod = 20, mult = 2.0, sensitivity = 150) {
    if (candles.length < slowLength + 10) return { bullish: false, bearish: false, value: 0, display: '⚪ WAE N/A' };
    const closes = candles.map(c => parseFloat(c[4]));
    const n = closes.length;

    // MACD (fast EMA - slow EMA)
    const emaFast1 = parseFloat(calculateEMA(candles.slice(-(fastLength+2)), fastLength));
    const emaSlow1 = parseFloat(calculateEMA(candles.slice(-(slowLength+2)), slowLength));
    const prevFast = parseFloat(calculateEMA(candles.slice(-(fastLength+3),-1), fastLength));
    const prevSlow = parseFloat(calculateEMA(candles.slice(-(slowLength+3),-1), slowLength));

    const t1 = (emaFast1 - emaSlow1) * sensitivity;
    const t2 = (prevFast - prevSlow) * sensitivity;

    // BB width
    const sma = closes.slice(-bbPeriod).reduce((s,v)=>s+v,0) / bbPeriod;
    const std = Math.sqrt(closes.slice(-bbPeriod).reduce((s,v)=>s+Math.pow(v-sma,2),0) / bbPeriod);
    const bbWidth = (sma + mult*std) - (sma - mult*std);

    const explosion = bbWidth * sensitivity / 10;
    const deadzone  = explosion * 0.1;
    const value     = t1 - t2;

    const bullish = t1 >= 0 && t1 > deadzone && t1 > explosion;
    const bearish = t1 <  0 && Math.abs(t1) > deadzone && Math.abs(t1) > explosion;

    const display = bullish ? `💥 Waddah Attar BULL Explosion 🚀 (+${t1.toFixed(1)})`
                  : bearish ? `💥 Waddah Attar BEAR Explosion 📉 (${t1.toFixed(1)})`
                  : t1 > deadzone ? '🟡 WAE: Weak Bull signal'
                  : t1 < -deadzone ? '🟡 WAE: Weak Bear signal'
                  : '⚪ WAE: No explosion (inside deadzone)';

    return { bullish, bearish, value: parseFloat(t1.toFixed(2)), explosion: parseFloat(explosion.toFixed(2)), display };
}

// ─────────────────────────────────────────────────────────────────
// Hull Moving Average (HMA) — fast, responsive, smooth
// ─────────────────────────────────────────────────────────────────
function calculateHMA(candles, period = 14) {
    if (candles.length < period * 2) return { value: 0, isBull: false, isBear: false, display: '⚪ HMA N/A' };
    const closes = candles.map(c => parseFloat(c[4]));

    function wma(data, len) {
        const d = data.slice(-len);
        let sum = 0, wSum = 0;
        for (let i = 0; i < d.length; i++) { const w = i+1; sum += d[i]*w; wSum += w; }
        return sum / wSum;
    }
    ...
    // Build diff series for WMA
    const diffSeries = [];
    for (let i = sqrtPeriod; i <= closes.length; i++) {
        const wf = wma(closes.slice(0, i), period);
        const wh = wma(closes.slice(0, i), halfPeriod);
        diffSeries.push(2 * wh - wf);
    }
    const hma     = wma(diffSeries.map((v,i)=>v), sqrtPeriod);
    const prevHma = diffSeries.length >= 2
        ? wma(diffSeries.slice(0,-1).map((v,i)=>v), sqrtPeriod)
        : hma;

    const isBull = hma > prevHma;
    const isBear = hma < prevHma;
    const cp     = closes[closes.length-1];

    return {
        value:   parseFloat(hma.toFixed(4)),
        isBull,
        isBear,
        aboveHMA: cp > hma,
        display: isBull ? `🟢 HMA Rising ($${hma.toFixed(4)})` : isBear ? `🔴 HMA Falling ($${hma.toFixed(4)})` : `⚪ HMA Flat ($${hma.toFixed(4)})`,
    };
}

// ─────────────────────────────────────────────────────────────────
// KAMA — Kaufman Adaptive Moving Average
// Fast in trending, slow in choppy — adapts to noise
// ─────────────────────────────────────────────────────────────────
function calculateKAMA(candles, period = 14, fastEnd = 2, slowEnd = 30) {
    if (candles.length < period + 5) return { value: 0, isBull: false, display: '⚪ KAMA N/A' };
    const closes = candles.map(c => parseFloat(c[4]));
    const n = closes.length;
    ...
    const prevKama = kama - (kama - closes[n-2]) * 0.1; // approximation for trend
    const cp       = closes[n-1];
    const isBull   = cp > kama && kama > prevKama;
    const isBear   = cp < kama && kama < prevKama;

    return {
        value:   parseFloat(kama.toFixed(4)),
        isBull, isBear,
        display: isBull ? `🟢 KAMA Bull ($${kama.toFixed(4)})`
               : isBear ? `🔴 KAMA Bear ($${kama.toFixed(4)})`
               : `⚪ KAMA Flat ($${kama.toFixed(4)})`,
    };
}

// ─────────────────────────────────────────────────────────────────
// VPCI — Volume Price Confirmation Indicator
// Confirms if price move is backed by volume
// ─────────────────────────────────────────────────────────────────
function calculateVPCI(candles, shortPeriod = 5, longPeriod = 25) {
    if (candles.length < longPeriod + 5) return { value: 0, confirmed: false, display: '⚪ VPCI N/A' };

    function vwma(data, len) {
        const d = data.slice(-len);
        let pv = 0, v = 0;
        for (const c of d) { const p = parseFloat(c[4]), vol = parseFloat(c[5]); pv += p*vol; v += vol; }
        return v > 0 ? pv / v : 0;
    }
    function sma(data, len) {
        return data.slice(-len).map(c=>parseFloat(c[4])).reduce((s,v)=>s+v,0) / len;
    }
    function volSma(data, len) {
        return data.slice(-len).map(c=>parseFloat(c[5])).reduce((s,v)=>s+v,0) / len;
    }

    const vpc  = vwma(candles, longPeriod)  - sma(candles, longPeriod);
    const vpr  = vwma(candles, shortPeriod) / sma(candles, shortPeriod);
    const vm   = volSma(candles, shortPeriod) / volSma(candles, longPeriod);
    const vpci = vpc * vpr * vm;

    const cp       = parseFloat(candles[candles.length-1][4]);
    const prevClose= parseFloat(candles[candles.length-2][4]);
    const bullMove  = cp > prevClose;
    const confirmed = (bullMove && vpci > 0) || (!bullMove && vpci < 0);

    return {
        value:     parseFloat(vpci.toFixed(4)),
        confirmed,
        isBull:    vpci > 0 && bullMove,
        isBear:    vpci < 0 && !bullMove,
        display:   confirmed ? (vpci > 0 ? '✅ VPCI: Volume CONFIRMS Bull move 🚀' : '✅ VPCI: Volume CONFIRMS Bear move 📉')
                              : (vpci > 0 ? '⚠️ VPCI: Bull price but weak volume' : '⚠️ VPCI: Bear price but weak volume'),
    };
}

// ─────────────────────────────────────────────────────────────────
// Stiffness Indicator
// Measures how strongly price holds above a baseline (SMA)
// High stiffness = strong trend, price keeps bouncing off MA
// ─────────────────────────────────────────────────────────────────
function calculateStiffness(candles, period = 60, smoothing = 3, threshold = 90) {
    if (candles.length < period + 10) return { value: 0, isStiff: false, isBull: false, display: '⚪ Stiffness N/A' };
    const closes = candles.slice(-(period + smoothing)).map(c => parseFloat(c[4]));
    const n = closes.length;

    // Count candles above SMA200 equivalent (use period/3 for speed)
    const smaPeriod = Math.floor(period / 3);
    let countAbove = 0;
    for (let i = smaPeriod; i < n; i++) {
        const smaVal = closes.slice(i - smaPeriod, i).reduce((s,v)=>s+v,0) / smaPeriod;
        if (closes[i] > smaVal) countAbove++;
    }
    const stiffness = (countAbove / (n - smaPeriod)) * 100;

    const cp     = closes[n-1];
    const smaVal = closes.slice(-smaPeriod).reduce((s,v)=>s+v,0) / smaPeriod;
    const isBull = stiffness >= threshold && cp > smaVal;
    const isBear = stiffness <= (100 - threshold) && cp < smaVal;

    return {
        value:   parseFloat(stiffness.toFixed(1)),
        isStiff: stiffness >= threshold,
        isBull, isBear,
        display: isBull ? `💪 Stiffness ${stiffness.toFixed(0)}% — Strong BULL trend`
               : isBear ? `💪 Stiffness ${stiffness.toFixed(0)}% — Strong BEAR trend`
               : `⚪ Stiffness ${stiffness.toFixed(0)}% — No clear dominance`,
    };
}

// ─────────────────────────────────────────────────────────────────
// Hurst Exponent (simplified rescaled range)
// H > 0.6 = trending, H < 0.4 = mean-reverting, H ≈ 0.5 = random
// ─────────────────────────────────────────────────────────────────
function calculateHurstExponent(candles, minPeriod = 8, maxPeriod = 32) {
    if (candles.length < maxPeriod * 2) return { value: 0.5, trending: false, meanReverting: false, display: '⚪ Hurst N/A' };
    const closes = candles.slice(-maxPeriod * 2).map(c => Math.log(parseFloat(c[4])));

    function rsCalc(data) {
        const n   = data.length;
        const mean = data.reduce((s,v)=>s+v,0) / n;
        const dev  = data.map(v => v - mean);
        let cumDev = 0, maxC = -Infinity, minC = Infinity;
        for (const d of dev) { cumDev += d; if (cumDev > maxC) maxC = cumDev; if (cumDev < minC) minC = cumDev; }
        const range = maxC - minC;
        const std   = Math.sqrt(dev.reduce((s,v)=>s+v*v,0) / n);
        return std > 0 ? range / std : 0;
    }

    const logRS = [], logN = [];
    for (let p = minPeriod; p <= maxPeriod; p += Math.floor((maxPeriod - minPeriod) / 4) || 1) {
        const rs = rsCalc(closes.slice(-p));
        if (rs > 0) { logRS.push(Math.log(rs)); logN.push(Math.log(p)); }
    }

    if (logRS.length < 2) return { value: 0.5, trending: false, meanReverting: false, display: '⚪ Hurst N/A' };

    // Linear regression slope = Hurst exponent
    const n    = logRS.length;
    const xMean = logN.reduce((s,v)=>s+v,0) / n;
    const yMean = logRS.reduce((s,v)=>s+v,0) / n;
    const num   = logRS.reduce((s,v,i)=>s+(logN[i]-xMean)*(v-yMean),0);
    const den   = logN.reduce((s,v)=>s+Math.pow(v-xMean,2),0);
    const H     = den > 0 ? parseFloat((num/den).toFixed(3)) : 0.5;

    const trending     = H > 0.6;
    const meanReverting = H < 0.4;

    return {
        value: H,
        trending, meanReverting,
        random: !trending && !meanReverting,
        display: trending      ? `📈 Hurst ${H} — Trending market (momentum strategies work)`
               : meanReverting ? `↩️ Hurst ${H} — Mean-reverting (fading extremes works)`
               : `🎲 Hurst ${H} — Random walk (no clear edge)`,
    };
}
...
module.exports = {
    ...
    calculateCCI,
    detectMomentumShift,
    calculateSqueezeMomentum, calculateWaddahAttar, calculateHMA,
    calculateKAMA, calculateVPCI, calculateStiffness, calculateHurstExponent,
};
