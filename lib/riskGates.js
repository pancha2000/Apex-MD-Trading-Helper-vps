...
async function getDailyPaperPnL(userId, db) {
    ...
    let trades;
    try {
        // Use existing db.Trade query pattern
        // We call the raw mongoose model via db if available
        if (db.Trade) {
            trades = await db.Trade
                .find({
                    userId:    userId,
                    isPaper:   true,
                    status:    'closed',
                ...
                .limit(DAILY_PNL_LOOKBACK_LIMIT)
                .select('pnlPct')
                .lean();
        } else if (typeof db.getDailyPaperPnL === 'function') {
            // If the db module already exposes this
            return await db.getDailyPaperPnL(userId);
        } else {
            // Fallback: assume 0 if we can't query
            return 0;
        }
    } catch (err) {
