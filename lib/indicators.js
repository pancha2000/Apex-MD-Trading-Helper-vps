...
function getMTFFibEntrySystem(candles15m, candles1H, candles4H, direction) {
    ...
    // Zone bypass check: if price already passed all nearby zones going to TP
    // i.e., price is between the zone and TP → zone was bypassed
    const zoneBypassed = isLong
        ? zones.some(z => currentPrice > z.price * 1.02 && tps.some(tp => currentPrice < tp.price))
        : zones.some(z => currentPrice < z.price * 0.98 && tps.some(tp => currentPrice > tp.price));
...
function getEntryQualityGate(currentPrice, mtfFib, ob, direction, confirmation, score) {
    ...
    // Zone was bypassed — price ran to TP area then came back to entry
    // This is a LOWER quality entry — the move already happened
    if (mtfFib?.zoneBypassed) {
        return {
            grade:    'INVALIDATED',
            action:   '🚫 ZONE BYPASSED',
            detail:   'Price already ran past entry zone toward TP. Re-entry risky — wait for new structure.',
            color:    'red',
            blockEntry: true,
        };
    }
