...
async function _run14FactorAnalysisImpl(coin, timeframe = '15m') {
    ...
    // 🆕 v8 patterns
    const mtfRSIDiv    = checkMTFRSIDivergence(currentCandles.slice(-60), candles1H?.slice(-60));
    const cypherPat    = checkCypherPattern(currentCandles.slice(-80));
    const abcdPat      = checkABCDPattern(currentCandles.slice(-60));
    const headShould   = checkHeadShoulders(currentCandles.slice(-80));
    const trianglePat  = checkTrianglePattern(currentCandles.slice(-40));
    const threeDrives  = checkThreeDrives(currentCandles.slice(-80));
    const fibTimeZone  = checkFibTimeZone(currentCandles.slice(-30));
    ...
    if      (gradeRaw >= 65 && mtfAlignCount >= 3 && confScore >= 8) {
        signalGrade = 'A+'; signalGradeEmoji = '🏆'; signalGradeLabel = 'ELITE SETUP';
    } else if (gradeRaw >= 50 && mtfAlignCount >= 2) {
