 ...
 * Retries up to 3 times with backoff — 4H data is critical for analysis.
 */
async function seedCache(symbol, tf) {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
        try {
            ...
            throw new Error(`Empty response attempt ${attempt}`);
        } catch (e) {
            if (attempt < 3) {
                await new Promise(r => setTimeout(r, 800 * attempt));
            } else {
                console.warn(`[WS-Seed] Failed after 3 attempts: ${symbol} ${tf} — ${e.message}`);
            }
        }
    }
}
...
async function getKlineDataFromCache(coin, timeframe, limit = 100) {
    ...
    // 4H needs at least 30 candles for reliable analysis — re-fetch if stale/thin
    const minRequired = timeframe === '4h' ? Math.min(30, limit) : Math.min(limit, 5);
    if (cached && cached.length >= minRequired) {
        return cached.length <= limit ? cached : cached.slice(-limit);
    }

    // Cache miss or insufficient — REST fetch with retry
    console.warn(`[Cache] ${cached ? 'Insufficient' : 'Miss'} for ${coin} ${timeframe} (have ${cached?.length||0}, need ${minRequired}) — fetching via REST.`);
    try {
        ...
        // Return cached data even if stale/thin, better than nothing
...
async function getKlineData(coin, timeframe, limit = 100) {
    // ✅ OHLCV CACHE: check local MongoDB first — saves ~80% Binance API calls
    try {
        const ohlcvCache = require('./ohlcv-cache');
        const cached = await ohlcvCache.getCached(coin, timeframe, limit);
        if (cached) return cached;
    } catch (_ce) { /* cache miss — fall through to API */ }
    ...
    return await withRetry(async () => {
        ...
        try {
            ...
            // ✅ OHLCV CACHE: store fresh data for next request
            try {
                const ohlcvCache = require('./ohlcv-cache');
                await ohlcvCache.setCache(coin, timeframe, limit, res.data);
            } catch (_se) { /* silent — cache write failure never aborts */ }
            return res.data;
        } catch (e) {
