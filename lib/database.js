...
const SettingsSchema = new mongoose.Schema({
    ...
    maxDailyLossPct:         { type: Number,  default: 10    },
    lowWinRateThreshold:     { type: Number,  default: 40    },
    lowWinRateLookback:      { type: Number,  default: 10    },
    backtestFeePct:          { type: Number,  default: 0.04  },
    backtestSlippagePct:     { type: Number,  default: 0.05  },
});
...
const ApiKeyEntrySchema = new mongoose.Schema({
    ...
    encSecretKey: { type: String, default: '' },       // AES-256-GCM ciphertext (optional — read-only mode if empty)
...
const SaasUserSchema = new mongoose.Schema({
    ...
    encGroqApiKey:    { type: String, default: '' }, // user's own GROQ key (encrypted)
    encGeminiApiKey:  { type: String, default: '' }, // user's own Gemini key (encrypted)
    tier:             { type: String, enum: ['free','vip'], default: 'free' }, // subscription tier
    minScoreThreshold: { type: Number, default: 20, min: 10, max: 90 },

    // ── Predictive Brain & Auto-Pilot (added for Brain Dashboard) ──────
    watchlist_coins: { type: mongoose.Schema.Types.Mixed, default: [] },
    tournament_history: { type: mongoose.Schema.Types.Mixed, default: [] },
    autopilot_global: { type: Boolean, default: false },
    cooldownUntil:    { type: Date,    default: null },
    maxDailyLossPct:  { type: Number,  default: 5 },
    predictive_brain_version: { type: String, default: 'v1' },

    // ── Signal Quality Filters (settings page) ──────────────────────────
    minRRR:              { type: Number,  default: 2.0  }, // minimum risk:reward ratio
    gradeFilter:         { type: String,  default: 'A'  }, // min grade: A+, A, B
    requireDailyAlign:   { type: Boolean, default: true  }, // block trades against daily trend
    maxConsecLosses:     { type: Number,  default: 3    }, // pause after N consecutive losses
    consecLossCooldown:  { type: Number,  default: 2    }, // hours to pause after hitting limit
    lowWinRateThreshold: { type: Number,  default: 40   }, // warn if win rate below X%
    trailingSl:          { type: Boolean, default: true  }, // move SL to entry after TP1
    fastFillOnly:        { type: Boolean, default: false }, // show only fast-fill limit setups
});
...
const TradeSchema = new mongoose.Schema({
    ...
    zoneStrength:{ type: String,  default: '' },
...
// ─────────────────────────────────────────────────────────────────
//  6. ADAPTIVE WEIGHTS SCHEMA
//  Stores weekly backtest-derived indicator performance scores.
//  Updated once per week by the adaptive weight engine.
//  Key = indicator name, value = win rate contribution.
// ─────────────────────────────────────────────────────────────────
const AdaptiveWeightSchema = new mongoose.Schema({
    week:       { type: String, required: true, unique: true }, // e.g. "2025-W12"
    weights:    { type: mongoose.Schema.Types.Mixed, default: {} }, // { indicatorName: weightMultiplier }
    sampleSize: { type: Number, default: 0 },   // trades analyzed
    winRate:    { type: Number, default: 0 },    // overall win rate for the week
    updatedAt:  { type: Date,   default: Date.now },
});
const AdaptiveWeight = mongoose.models.AdaptiveWeight || mongoose.model('AdaptiveWeight', AdaptiveWeightSchema);
...
async function setUserTier(userId, tier) {
    await connect();
    if (!['free','vip'].includes(tier)) throw new Error('Invalid tier');
    await SaasUser.findByIdAndUpdate(userId, { tier });
}

async function deleteSaasUser(userId) {
    await connect();
    await SaasUser.findByIdAndDelete(userId);
    // Also clean up their trades
    await Trade.deleteMany({ userId });
}

async function setUserAiKey(userId, type, encKey) {
    await connect();
    const field = type === 'groq' ? 'encGroqApiKey' : 'encGeminiApiKey';
    await SaasUser.findByIdAndUpdate(userId, { [field]: encKey });
}
...
async function setUserMinScore(userId, score) {
    await connect();
    await SaasUser.findByIdAndUpdate(userId, { minScoreThreshold: score });
}

// ── Predictive Brain: Watchlist CRUD ─────────────────────────────────────
async function addWatchlistCoin(userId, coinData) {
    await connect();
    const user = await SaasUser.findById(userId);
    if (!user) return null;
    user.watchlist_coins = [...(user.watchlist_coins || []), coinData];
    user.markModified('watchlist_coins');
    return user.save();
}
...
async function removeWatchlistCoin(userId, coinPair) {
    ...
    const user = await SaasUser.findById(userId);
    if (!user) return null;
    user.watchlist_coins = (user.watchlist_coins || []).filter(c => c.coin_pair !== coinPair);
    user.markModified('watchlist_coins');
    return user.save();
}
...
async function updateWatchlistCoin(userId, coinPair, fields) {
    ...
    const idx = coins.findIndex(c => c.coin_pair === coinPair);
    if (idx === -1) return null;
    Object.assign(coins[idx], fields);
    user.markModified('watchlist_coins');
    return user.save();
}
...
async function updateSaasUser(userId, fields) {
    await connect();
    return SaasUser.findByIdAndUpdate(userId, { $set: fields }, { new: true });
}

async function getProtectionSettings() {
    const s = await getSettings();
    return {
        enabled:             s.protectionEnabled ?? true,
        cooldownHours:       s.cooldownHours      ?? 2,
        maxDailyLossPct:     s.maxDailyLossPct    ?? 10,
        lowWinRateThreshold: s.lowWinRateThreshold ?? 40,
        lowWinRateLookback:  s.lowWinRateLookback  ?? 10,
    };
}
async function updateProtectionSettings(patch) {
    await connect();
    const allowed = ['protectionEnabled','cooldownHours','maxDailyLossPct','lowWinRateThreshold','lowWinRateLookback'];
    const update  = {};
...
async function getBacktestSettings() {
    const s = await getSettings();
    return { feePct: s.backtestFeePct ?? 0.04, slippagePct: s.backtestSlippagePct ?? 0.05 };
}
// ── Adaptive Weight helpers ──────────────────────────────────────────────
async function getAdaptiveWeights() {
    // Get current week key e.g. "2025-W12"
    const now  = new Date();
    const week = `${now.getUTCFullYear()}-W${String(Math.ceil((now.getUTCDate() - now.getUTCDay() + 10) / 7)).padStart(2,'0')}`;
    try {
        const doc = await AdaptiveWeight.findOne({ week }).lean();
        return doc ? { week, weights: doc.weights, sampleSize: doc.sampleSize, winRate: doc.winRate } : null;
    } catch(_) { return null; }
}

async function saveAdaptiveWeights(weights, sampleSize, winRate) {
    const now  = new Date();
    const week = `${now.getUTCFullYear()}-W${String(Math.ceil((now.getUTCDate() - now.getUTCDay() + 10) / 7)).padStart(2,'0')}`;
    try {
        await AdaptiveWeight.findOneAndUpdate(
            { week },
            { weights, sampleSize, winRate, updatedAt: new Date() },
            { upsert: true }
        );
        return week;
    } catch(_) { return null; }
...
module.exports = {
    ...
    getAdaptiveWeights,
    saveAdaptiveWeights,
    AdaptiveWeight, getMargin, setMargin, saveMargin: setMargin,
    ...
    setUserTier,
    deleteSaasUser,
    getProtectionSettings, updateProtectionSettings, getBacktestSettings,
    setUserMinScore,
    // Predictive Brain
    addWatchlistCoin, removeWatchlistCoin, updateWatchlistCoin,
    updateWatchlistCoinStrategy, updateSaasUser,
