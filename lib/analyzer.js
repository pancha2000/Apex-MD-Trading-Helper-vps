...
async function _run14FactorAnalysisImpl(coin, timeframe = '15m') {
    ...
    // ── 1. Data Fetching ──────────────────────────────────────
    const [currentCandles, candles5m, candles1H, candles4H, candlesDaily, fundingRaw] = await Promise.all([
    ...
    // ══════════════════════════════════════════════════════════
    // ── Funding Rate Quality Gate ──────────────────────────────────────
    // Extreme positive funding = longs getting squeezed soon → penalise LONG
    // Extreme negative funding = shorts getting squeezed soon → penalise SHORT
    if (fundingRate !== null) {
        if (fundingRate > 0.1)  { longScore  = Math.max(0, longScore  - 3); longR.push(`⚠️ High Funding ${fundingRate.toFixed(3)}%`); }
        if (fundingRate > 0.2)  { longScore  = Math.max(0, longScore  - 5); }   // extreme
        if (fundingRate < -0.1) { shortScore = Math.max(0, shortScore - 3); shortR.push(`⚠️ Neg Funding ${fundingRate.toFixed(3)}%`); }
        if (fundingRate < -0.2) { shortScore = Math.max(0, shortScore - 5); }   // extreme
        // Contrarian: very extreme positive funding = SHORT squeeze play
    ...
    const bestDirection = scoreDiff >= 5 ? smartDir : direction;  // need clear edge
    ...
    // ✅ FIX: HTF confluence required — if 1H and 4H disagree, penalty
    ...
    // ✅ Session gate — Asian session penalty
    const _utcHour  = new Date().getUTCHours();
    const _isAsian  = _utcHour >= 0 && _utcHour < 6;
    const _isWeekend = [0,6].includes(new Date().getUTCDay());
    if (_isAsian) {
        longScore  = Math.max(0, longScore  - 10);
        shortScore = Math.max(0, shortScore - 10);
        longR.push('⚠️ Asian Session (-10)');
        shortR.push('⚠️ Asian Session (-10)');
    }
    if (_isWeekend) {
        longScore  = Math.max(0, longScore  - 5);
        shortScore = Math.max(0, shortScore - 5);
        longR.push('⚠️ Weekend Low Liquidity (-5)');
        shortR.push('⚠️ Weekend Low Liquidity (-5)');
    }
    // ✅ BTC correlation gate
    if (coin !== 'BTCUSDT') {
        const btcBearish = trend4H.includes('Bearish') && trend1H.includes('Bearish');
        const btcBullish = trend4H.includes('Bullish') && trend1H.includes('Bullish');
        if (btcBearish && direction === 'LONG')  { longScore  = Math.max(0, longScore  - 5); longR.push('⚠️ BTC Headwind LONG (-5)'); }
        if (btcBullish && direction === 'SHORT') { shortScore = Math.max(0, shortScore - 5); shortR.push('⚠️ BTC Headwind SHORT (-5)'); }
    }
