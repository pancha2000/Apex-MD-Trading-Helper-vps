...
function checkABCDPattern(candles) {
    if (candles.length < 30) return { bull: false, bear: false, display: 'None' };
    const highs = candles.map(c => parseFloat(c[2]));
    const lows  = candles.map(c => parseFloat(c[3]));
    const n     = candles.length;
    const curP  = parseFloat(candles[n-1][4]);
    ...
    // Bullish ABCD: A(high) B(low) C(high) D(low=entry)
    if (swHL.length >= 2 && swLL.length >= 2) {
        const A = swHL[swHL.length-2].v, Ai = swHL[swHL.length-2].i;
        const B = swLL[swLL.filter(p=>p.i>Ai)[0]?.i === undefined ? 0 : swLL.findIndex(p=>p.i>Ai)];
        if (B) {
            const Bv = B.v;
            const AB = A - Bv;
            const C  = swHL.find(p => p.i > B.i)?.v;
            if (C && AB > 0) {
                const BC = C - Bv;
                const BCratio = BC / AB;
                if (BCratio >= 0.618 - tol && BCratio <= 0.786 + tol) {
                    ...
                    if (curP <= Math.max(D_127, D_161) * 1.015 && curP >= Math.min(D_127, D_161) * 0.985) {
                        return { bull: true, bear: false, display: '🟢 Bullish ABCD Pattern PRZ' };
                    }
                }
            }
        }
